...
int Table::Load(::client::Session* session, ::osd::common::ObjectId oid, ::client::Table** ipp)
{
    int ret = E_SUCCESS;
...
int Table::Lock(::client::Session* session, lock_protocol::Mode mode)
{
...
int Table::Unlock(::client::Session* session)
{
    osd::containers::client::NameContainer::Proxy* cc_proxy;
...
int Table::Put(::client::Session* session, const char* key, const char* src, uint64_t n)
{
    int ret;
    ...
    if ((ret = rw_ref()->proxy()->interface()->Find(session, key, &oid)) != E_SUCCESS)
    {
        ...
        session->journal() << Publisher::Message::LogicalOperation::MakeFile(
            rw_ref()->proxy()->oid().u64(), key, obj->oid().u64());
...
int Table::Get(::client::Session* session, const char* key, char* dst)
{
    int ret;
    osd::common::ObjectId oid;
    osd::containers::client::NeedleContainer::Object* obj;
...
int Table::Erase(::client::Session* session, const char* key)
{
    int ret;
    osd::common::ObjectId oid;
    osd::containers::client::NeedleContainer::Object* obj;
    ...
    session->journal()->TransactionBegin();
    session->journal() << Publisher::Message::LogicalOperation::Unlink(
        rw_ref()->proxy()->oid().u64(), key);
...
int Table::Lock(::client::Session* session, int name_container, lock_protocol::Mode mode)
{
    osd::containers::client::NameContainer::Proxy* cc_proxy;
...
int Table::Unlock(::client::Session* session, int name_container)
{
    osd::containers::client::NameContainer::Proxy* cc_proxy;
...
int Table::Put(::client::Session* session, const char* key, const char* src, uint64_t n)
{
    ...
    if ((ret = rw_ref->proxy()->interface()->Find(session, key, &oid)) != E_SUCCESS)
    {
        ...
        // session->journal() <<
        // Publisher::Message::LogicalOperation::MakeFile(rw_ref->proxy()->oid().u64(), key,
        // obj->oid().u64());
        session->journal()->TransactionCommit();
    }
    else
...
int Table::Erase(::client::Session* session, const char* key)
{
    ...
    session->journal()->TransactionBegin();
    session->journal() << Publisher::Message::LogicalOperation::Unlink(rw_ref->proxy()->oid().u64(),
                                                                       key);
    if ((ret = rw_ref->proxy()->interface()->Erase(session, key)) < 0)
    {
        goto done;
