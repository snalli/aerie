...
static int test_multiblock()
{
    ...
    for (int i = 0; i < N; i++) w[i] = (char)((i * 31 + 7) & 0xFF);
    memset(r, 0, N);

    int rc = 0;
    int fd = libfs_open2("/pxfs/t_multi.dat", O_CREAT|O_RDWR|O_TRUNC, 0644);
    if (fd <= 0)                        { rc = -1; goto done; }
    if (libfs_write(fd, w, N) != N)     { rc = -1; goto done; }
    if (libfs_fsync(fd) != 0)           { rc = -1; goto done; }
    if (libfs_lseek(fd, 0, SEEK_SET)!=0){ rc = -1; goto done; }
    if (libfs_read(fd, r, N) != N)      { rc = -1; goto done; }
    if (memcmp(r, w, N) != 0)           { rc = -1; goto done; }
    if (libfs_close(fd) != 0)           { rc = -1; goto done; }
done:
    free(w); free(r);
    return rc;
}
