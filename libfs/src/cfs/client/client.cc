...
int Client::Init(const char* xdst)
{
    int ret;
...
//
static inline int create(const char* path, int mode, int type, InodeNumber* inop)
{
    int ret;
    ...
    {
    case kDirInode:
        if ((ret = global_ipc_layer->call(FileSystemProtocol::kMakeDir, global_ipc_layer->id(),
                                          std::string(path), (unsigned int) mode, ino)) < 0)
        {
            return -E_IPC;
        }
        if (ret > 0)
        ...
        *inop = ino;
        ...
        break;
    case kFileInode:
        if ((ret = global_ipc_layer->call(FileSystemProtocol::kMakeFile, global_ipc_layer->id(),
                                          std::string(path), (unsigned int) 0, (unsigned int) mode,
                                          ino)) < 0)
        {
            return -E_IPC;
        }
...
int Client::Open(const char* path, int flags, int mode)
{
    ...
    {
        if ((ret = global_ipc_layer->call(FileSystemProtocol::kNamei, global_ipc_layer->id(),
                                          std::string(path), protocol_ino)) < 0)
        {
            return -E_IPC;
        }
        if (ret > 0)
...
int Client::Link(const char* oldpath, const char* newpath)
{
    ...
    if ((ret = global_ipc_layer->call(FileSystemProtocol::kLink, global_ipc_layer->id(),
                                      std::string(oldpath), std::string(newpath), reply)) < 0)
    {
        return -E_IPC;
    }
    if (ret > 0)
...
int Client::Unlink(const char* path)
{
    ...
    if ((ret = global_ipc_layer->call(FileSystemProtocol::kUnlink, global_ipc_layer->id(),
                                      std::string(path), reply)) < 0)
    {
        return -E_IPC;
    }
    if (ret > 0)
