...
int DirInode::return_dentry(::client::Session* session, void* inode_list_head)
{
...
int DirInode::Lookup(::client::Session* session, const char* name, int /*flags*/,
                     ::client::Inode** ipp)
{
    PROFILER_PREAMBLE
    int ret;
    ...
    DBG_LOG(DBG_INFO, DBG_MODULE(client_inode), "Inode [%p, %lx]: lookup %s\n", this, ino(), name);
    ...
    // insight : DO ME : Do not proceed to LoadInode if we have a successful lookup in the
    // ShadowCache
    //
    PROFILER_SAMPLE
...
int DirInode::xLookup(::client::Session* session, const char* name, int /*flags*/,
                      ::client::Inode** ipp)
{
    int ret;
    ::client::Inode* ip;
    ...
    DBG_LOG(DBG_INFO, DBG_MODULE(client_inode), "Inode [%p, ino=%lx]: xLookup %s \n", this, ino(),
            name);
...
int DirInode::Link(::client::Session* session, const char* name, ::client::Inode* ip,
                   bool /*overwrite*/)
{
    int ret;

    DBG_LOG(DBG_INFO, DBG_MODULE(client_inode), "Inode [%p, %lx]: link %s -> (%p, ino=%lx)\n", this,
            ino(), name, ip, ip->ino());
...
int DirInode::Unlink(::client::Session* session, const char* name)
{
    int ret;

    DBG_LOG(DBG_INFO, DBG_MODULE(client_inode), "Inode [%p, %lx]: unlink %s\n", this, ino(), name);
...
int DirInode::nlink()
{
    DBG_LOG(DBG_INFO, DBG_MODULE(client_inode), "In inode %lx, nlink = %d\n", ino(),
            rw_ref()->proxy()->interface()->nlink());
...
{
    DBG_LOG(DBG_INFO, DBG_MODULE(client_inode), "In inode %lx, set nlink = %d\n", ino(), nlink);

    return rw_ref()->proxy()->interface()->set_nlink(nlink);
}

int DirInode::Lock(::client::Session* session, lock_protocol::Mode mode)
{
...
int DirInode::Lock(::client::Session* session, Inode* parent_inode, lock_protocol::Mode mode)
{
#ifdef FAST_SPIDER_LOCK_PATH
    pthread_mutex_lock(&spider_lock);
#else
...
int DirInode::Unlock(::client::Session* session)
{
#ifdef FAST_SPIDER_LOCK_PATH
...
int DirInode::xOpenRO(::client::Session* /*session*/)
{
    rw_ref()->proxy()->xOpenRO();
    return E_SUCCESS;
}

int DirInode::Sync(::client::Session* session)
{
    return rw_ref()->proxy()->interface()->vUpdate(session);
}

int DirInode::ioctl(::client::Session* session, int request, void* info)
{
    int ret = E_SUCCESS;
    switch (request)
    {
