...
static osd::cc::client::LockId root(1, 1);
static osd::cc::client::LockId a(1, 2);
static osd::cc::client::LockId b(1, 3);
static osd::cc::client::LockId c(1, 4);
...
SUITE(OSD_HLock)
{
    ...
    TEST_FIXTURE(LockFixture, TestLockIXLockXLUnlock)
    {
        ...
        EVENT("E1");
        printf("START\n");
        CHECK(global_storage_system->hlckmgr()->Acquire(root, lock_protocol::Mode::IX, 0) ==
              lock_protocol::OK);
        printf("DONE\n");
        EVENT("E2");
        CHECK(global_storage_system->hlckmgr()->Acquire(a, root, lock_protocol::Mode::XL, 0) ==
              lock_protocol::OK);
        CHECK(check_grant_x(region_, a.number()) == 0);
        global_storage_system->hlckmgr()->Release(a);
        CHECK(check_release(region_, a.number()) == 0);
    ...
    TEST_FIXTURE(LockFixture, TestLockIXLockIXLockXLUnlockAll)
    {
        ...
        CHECK(global_storage_system->hlckmgr()->Acquire(root, lock_protocol::Mode::IX, 0) ==
              lock_protocol::OK);
        EVENT("E2");
        CHECK(global_storage_system->hlckmgr()->Acquire(a, root, lock_protocol::Mode::IX, 0) ==
              lock_protocol::OK);
        EVENT("E3");
        CHECK(global_storage_system->hlckmgr()->Acquire(b, a, lock_protocol::Mode::XL, 0) ==
              lock_protocol::OK);
        EVENT("E4");
    ...
    TEST_FIXTURE(LockFixture, TestLockXR)
    {
        ...
        EVENT("E1");
        CHECK(global_storage_system->hlckmgr()->Acquire(root, lock_protocol::Mode::XR, 0) ==
              lock_protocol::OK);
        EVENT("E2");
    }
}
