...
#define ALL_RPC_NUMBER(module, protocol, handle) module##_##protocol##_##handle,

#define RPC_NUMBER(module, protocol, handle)                                                       \
    handle = StamnosGlobalRpcNumbers::module##_##protocol##_##handle,

#define DEFINE_RPC_NUMBER(ACTION) ACTION(RPC_NUMBER)
...
#define BCS_SHARED_BUFFER_PROTOCOL(ACTION) ACTION(bcs, SharedBufferProtocol, kConsume)
...
#define OSD_PUBLISHER_PROTOCOL(ACTION) ACTION(osd, PublisherProtocol, kPublish)

#define OSD_STORAGESYSTEM_PROTOCOL(ACTION) ACTION(osd, StorageSystemProtocol, kMount)

#define PXFS_FILESYSTEM_PROTOCOL(ACTION) ACTION(pxfs, FileSystemProtocol, kMount)

#define KVFS_FILESYSTEM_PROTOCOL(ACTION) ACTION(kvfs, FileSystemProtocol, kMount)
...
class StamnosGlobalRpcNumbers
{
  public:
    ...
    {
        null_rpc = 0x4000,
        BCS_IPC_PROTOCOL(ALL_RPC_NUMBER) BCS_SHARED_BUFFER_PROTOCOL(ALL_RPC_NUMBER)
            OSD_LOCK_PROTOCOL(ALL_RPC_NUMBER) OSD_RLOCK_PROTOCOL(ALL_RPC_NUMBER)
                OSD_STORAGE_PROTOCOL(ALL_RPC_NUMBER) OSD_REGISTRY_PROTOCOL(ALL_RPC_NUMBER)
