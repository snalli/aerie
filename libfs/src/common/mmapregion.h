...
template <typename HeaderT> class MemoryMappedRegion
{
  public:
    enum Flags
    {
        kCreate = 0x1,    /**< Creates the file backing the region */
        kExclusive = 0x2, /**< When combined with kCreate, the region is only created if it doesn't
                             already exist */
...
int MemoryMappedRegion<HeaderT>::CreateInternal(const char* pathname, size_t nblocks,
...
template <typename HeaderT>
int MemoryMappedRegion<HeaderT>::Create(const char* pathname, size_t size)
{
 ...
 */
template <typename HeaderT>
int MemoryMappedRegion<HeaderT>::Open(const char* pathname, size_t size, int flags,
                                      MemoryMappedRegion* region)
{
    ...
    if (flags & MemoryMappedRegion::kCreate)
    {
        ...
        {
            size_t nblocks = NumOfBlocks(size, kBlockSize);
            if ((ret = MemoryMappedRegion::CreateInternal(pathname, nblocks, kBlockSize, kBlockSize,
                                                          flags)) < 0)
            {
...
template <typename HeaderT>
int MemoryMappedRegion<HeaderT>::Open(const char* pathname, MemoryMappedRegion** region)
{
    return Open(pathname, 0, 0, region);
}

template <typename HeaderT> int MemoryMappedRegion<HeaderT>::Close(MemoryMappedRegion* region)
{
...
template <typename HeaderT> int MemoryMappedRegion<HeaderT>::Lock()
{
...
template <typename HeaderT> int MemoryMappedRegion<HeaderT>::Unlock()
{
...
template <typename HeaderT> class MemoryMappedRegion<HeaderT>::Header : public HeaderT
{
  public:
    Header() : initialized_(false), header_size_(0), payload_base_(0), payload_size_(0)
    {
    }

    Header(uint64_t header_size, uint64_t payload_size)
        : initialized_(true), header_size_(header_size), payload_base_(0),
    ...
    void set_payload_base(uint64_t payload_base)
    {
  ...
  protected:
    bool initialized_;
    uint64_t header_size_; // length of the space occupied by the header and extra space to properly
                           // align the region
    uint64_t payload_base_; // the address where the region payload was mapped (used by persistent
                            // region)
    uint64_t payload_size_;
};

template <typename HeaderT> int MemoryMappedRegion<HeaderT>::Header::Load(int fd, Header* header)
{
...
template <typename HeaderT> int MemoryMappedRegion<HeaderT>::Header::Store(int fd, Header& header)
{
