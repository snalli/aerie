...
rpcc::rpcc(string bind_f) : bind_done_(false), client_id(0)
{
...
int rpcc::bind()
{
    ...
    if (ret == 0)
    {
        ...
        DBG_LOG(DBG_DEBUG, DBG_MODULE(rpc),
                "rpcc bind successful, shared-file: %s, client id - %d\n", sh_chan_shf.c_str(),
                client_id);
    }
    else
    {
...
rpcs::rpcs(const char* bind_f) : bind_f_(bind_f)
{
    set_rand_seed();
...
void rpcs::dispatch(char* buf, unsigned int& sz /*return buffer is the same*/)
{
    ...
    assert(rh.ret >= 0 || rh.ret == rpc_const::unmarshal_args_failure);
...
void rpcs::handle_comm_q(server_lock_t* rpc_serv)
{
    ...
    if ((now_serv == MAX_TICKETS - 1) && (rpc_serv->rpc_comm_l->ticket) > now_serv)
    { // boundary check!
    ...
    if (rpc_serv->rpc_comm_sig[now_serv].signal != 0)
    { // client is not already signaled
...
void rpcs::handle_send_q(server_lock_t* rpc_serv, unsigned tid)
{
    ...
    for (curr = (rpc_serv->sendq.begin()); curr != rpc_serv->sendq.end(); curr++)
    {
        ...
        // check checksum
        if (chkChksum(currqe->rpc_request->data, currqe->rpc_request->sizeInChar,
...
void* rpcs::rpc_server_kernel(void* arg)
{
    ...
    while (1)
    {
        ...
        for (myturn = 0, curr = rpc_reg.rpc_queue.begin(); curr != rpc_reg.rpc_queue.end();
             curr++, myturn++)
        {
        ...
        for (curr = rpc_reg.rpc_queue.begin(); curr != rpc_reg.rpc_queue.end(); curr++)
        {
            s_ce = *curr;
