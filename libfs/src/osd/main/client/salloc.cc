...
int DescriptorPool::LoadFromLast(OsdSession* session)
{
    osd::common::ObjectId oid;
    osd::common::ExtentId eid;
...
int DescriptorPool::Create(::client::Ipc* ipc, OsdSession* session,
                           osd::common::AclIdentifier acl_id, DescriptorPool** poolp)
{
    ::osd::StorageProtocol::ContainerReply reply;
    DescriptorPool* pool;
    int ret;

    DBG_LOG(DBG_INFO, DBG_MODULE(client_salloc), "[%d] Allocate a pool of containers with ACL %d\n",
            ipc->id(), acl_id);

    if ((ret = ipc->call(osd::StorageProtocol::kAllocateObjectIdSet, ipc->id(), acl_id, reply)) < 0)
    {
        return ret;
    }
    else if (ret > 0)
...
int DescriptorPool::AllocateContainer(::client::Ipc* ipc, OsdSession* session, int type,
                                      osd::common::ObjectId* oid)
{
    int r;
    int ret;
    ...
    {
        if ((ret = ipc->call(osd::StorageProtocol::kAllocateContainerIntoSet, ipc->id(),
                             capability_, type, 1024, r)) < 0)
        {
            return ret;
        }
        else if (ret > 0)
    ...
    *oid = front.oid_;
    session->journal() << Publisher::Message::LogicalOperation::AllocContainer(
        capability_, front.oid_, front.index_);
...
int DescriptorPool::AllocateExtent(::client::Ipc* ipc, OsdSession* session, size_t /*nbytes*/,
                                   osd::common::ExtentId* eid)
{
    int ret;
    int r;
    ...
    {
        if ((ret = ipc->call(osd::StorageProtocol::kAllocateExtentIntoSet, ipc->id(), capability_,
                             4096, 1024, r)) < 0)
        {
            return ret;
        }
    ...
    *eid = front.eid_;
    session->journal() << osd::Publisher::Message::ContainerOperation::AllocateExtent(
        capability_, front.eid_, front.index_);
    extent_list_.pop_front();
    return E_SUCCESS;
}

int StorageAllocator::Load(StoragePool* pool)
{
    pool_ = pool;
    return E_SUCCESS;
}
                                        ...
                                        DescriptorPool** poolp)
{
    ...
    DBG_LOG(DBG_INFO, DBG_MODULE(client_salloc), "[%d] Allocate container set of ACL %d\n",
            ipc_->id(), acl_id);
...
int StorageAllocator::AllocateExtent(OsdSession* session, osd::common::AclIdentifier acl_id,
                                     size_t nbytes, osd::common::ExtentId* eidp, bool has_lock)
{
    ...
    DBG_LOG(DBG_INFO, DBG_MODULE(client_salloc), "[%d] Allocate extent of size %lu\n", ipc_->id(),
            nbytes);
...
int StorageAllocator::AllocateExtent(OsdSession* session, osd::common::AclIdentifier acl_id,
                                     size_t nbytes, int flags, void** ptr)
{
    int ret;
    osd::common::ExtentId eid;
...
int StorageAllocator::AllocateExtent(OsdSession* session, size_t nbytes, int flags, void** ptr)
{
                                        ...
                                        int type, osd::common::ObjectId* oidp)
{
    ...
    DBG_LOG(DBG_INFO, DBG_MODULE(client_salloc), "[%d] Allocate container of type %d\n", ipc_->id(),
            type);
...
int StorageAllocator::AllocateContainerVector(OsdSession* /*session*/)
{
    ...
    (void) ipc_->call(osd::StorageProtocol::kAllocateContainerVector, ipc_->id(), container_req_vec,
                      rv);
