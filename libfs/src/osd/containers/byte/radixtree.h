...
template <typename Session> class RadixTreeNode
{
  public:
    RadixTreeNode()
    {
    ...
    static RadixTreeNode* Make(Session* session)
    {
        ...
        if ((ret = session->salloc()->AllocateExtent(session, sizeof(RadixTreeNode), kMetadata,
                                                     &ptr)) < 0)
        {
    ...
    {
        RadixTreeNode* n;
        n = reinterpret_cast<RadixTreeNode*>(
            radix_tree_indirect_to_ptr(reinterpret_cast<void*>(node)));
...
template <typename Session> class RadixTree
{
  public:
    RadixTree();
    ...
    int LowerBoundInternal(Session* session, uint64_t index, RadixTreeNode<Session>* node,
                           uint64_t low, unsigned int h, uint64_t* lge_index);

    inline RadixTreeNode<Session>* get_rnode()
    {
        return rnode_;
    }
...
template <typename Session> RadixTree<Session>::RadixTree() : height_(0), rnode_(NULL)
{
}

static uint64_t __maxindex(unsigned int height)
{
...
static void radix_tree_init_maxindex(uint64_t height_to_maxindex[], uint64_t array_size)
{
    unsigned int i;
 ...
 */
template <typename Session> uint64_t RadixTree<Session>::MaxIndex(unsigned int height)
{
    /*
 ...
 */
template <typename Session> int RadixTree<Session>::Extend(Session* session, uint64_t index)
{
    ...
    do
    {
        ...
        newheight = height_ + 1;
        node = reinterpret_cast<RadixTreeNode<Session>*>(
            radix_tree_ptr_to_indirect(reinterpret_cast<void*>(node)));
        rnode_ = node;
        height_ = newheight;
    } while (h > height_);
...
int RadixTree<Session>::MapSlot(Session* session, uint64_t index, int min_height, int alloc,
                                RadixTreeNode<Session>** result_node, int* result_offset,
                                int* result_height)
{
    ...
    slot = reinterpret_cast<RadixTreeNode<Session>*>(
        radix_tree_indirect_to_ptr(reinterpret_cast<void*>(rnode_)));
    h = height_;
    ...
    while (h > 0)
    {
        if (slot == NULL)
        {
            if (alloc)
            {
                ...
                {
                    rnode_ = reinterpret_cast<RadixTreeNode<Session>*>(
                        radix_tree_ptr_to_indirect(reinterpret_cast<void*>(slot)));
                }
            }
            else
            {
...
template <typename Session>
int RadixTree<Session>::LowerBoundInternal(Session* session, uint64_t index,
                                           RadixTreeNode<Session>* node, uint64_t low,
                                           unsigned int h, uint64_t* lge_index)
{
    unsigned int shift;
    ...
    for (int i = 0; i < RADIX_TREE_MAP_SIZE; i++)
    {
        ...
        if (node->Slot(session, i))
        {
            ...
            {
                if (LowerBoundInternal(session, index, node->Slot(session, i), low + (i << shift),
                                       h - 1, lge_index) == 0)
                {
                    return 0;
                }
...
template <typename Session>
int RadixTree<Session>::LowerBound(Session* session, uint64_t index, uint64_t* lge_index)
{
    RadixTreeNode<Session>* slot;
...
///
template <typename Session>
int RadixTree<Session>::Insert(Session* session, uint64_t index, void* item, int min_height)
{
    RadixTreeNode<Session>* node;
    RadixTreeNode<Session>* slot;
    int offset;
...
///
template <typename Session>
void** RadixTree<Session>::LookupSlot(Session* session, uint64_t index, int min_height)
{
    RadixTreeNode<Session>** slot;
    RadixTreeNode<Session>* node;
    int offset;
...
///
template <typename Session>
void* RadixTree<Session>::Lookup(Session* session, uint64_t index, int min_height)
{
    RadixTreeNode<Session>** slot;
    RadixTreeNode<Session>* node;
    int offset;
...
#endif /* __STAMNOS_OSD_COMMON_RADIXTREE_H */
