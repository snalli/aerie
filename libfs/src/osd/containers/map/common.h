...
int MapContainer::Object<Session>::Find(Session* session, const char* name,
                                        osd::common::ObjectId* oid)
{
    uint64_t u64;
    int ret;
...
template <typename Session>
int MapContainer::Object<Session>::Insert(Session* session, const char* name,
                                          osd::common::ObjectId oid)
{
    uint64_t u64;
...
template <typename Session>
int MapContainer::Object<Session>::Erase(Session* session, const char* name)
{
    if (name[0] == '\0')
    {
        return -1;
...
template <typename Session> int MapContainer::Object<Session>::Size(Session* session)
{
    int size = 0;
