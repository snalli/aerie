...
HLockManager::HLockManager(::server::Ipc* ipc) : ipc_(ipc), lm_(NULL)
{
}

int HLockManager::Init()
{
    DBG_LOG(DBG_INFO, DBG_MODULE(server_hlckmgr), "Initializing Hierachical Lock Manager: %p\n",
            this);
...
lock_protocol::status HLockManager::Acquire(int clt, int seq, lock_protocol::LockId lid,
                                            int mode_seti, int flags, unsigned long long arg,
                                            int& mode_granted)
{
    ...
    {
        if ((plock = lm_->FindOrCreateLockInternal(plid)) && (cr = plock->gtque_.Find(clt)))
        {
            plock_mode = cr->mode();
        }
        else
        ...
        DBG_LOG(DBG_INFO, DBG_MODULE(server_hlckmgr),
                ...
                clt, seq, LockId(lid).c_str(), mode_set.String().c_str(), LockId(plid).c_str(),
                plock_mode.String().c_str());
    ...
    r = lm_->AcquireInternal(clt, seq, lock, lock_protocol::Mode::Set(mode_set), flags,
                             mode_granted);
done:
...
lock_protocol::status HLockManager::Convert(int clt, int seq, lock_protocol::LockId lid,
                                            int new_mode, int flags, int& unused)
{
    return lm_->Convert(clt, seq, lid, new_mode, flags, unused);
}

lock_protocol::status HLockManager::Release(int clt, int seq, lock_protocol::LockId lid, int flags,
                                            int& unused)
{
    return lm_->Release(clt, seq, lid, flags, unused);
}

lock_protocol::status HLockManager::Stat(lock_protocol::LockId lid, int& r)
{
...
HLockManager::Acquire(int clt, Lock& lock, lock_protocol::Mode mode, int flags, int argc, void**
argv)
{
