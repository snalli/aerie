...
class ObjectId
{
  ...
  public:
    ObjectId(uint64_t u64 = 0) : u64_(u64)
    {
    }

    ObjectId(const ObjectId& oid) : u64_(oid.u64_)
    {
    }
...
{
  public:
    ExtentId(uint64_t u64 = 0) : ObjectId(u64)
    {
    }

    ExtentId(osd::common::ObjectId oid) : ObjectId(oid.u64())
    {
    }

    ExtentId(void* addr, int /*size*/) : ObjectId(T_EXTENT, addr)
    {
    }
};
...
class Object
{
  public:
    ...
    Object() : type_(0), nlink_(0), dlink_(0), parent_(ObjectId(0))
    {
    }
...
{
  public:
    Object() : status_(0), version_(0)
    {
    }

    VersionNumber ccVersion()
    {
        return version_;
    }
