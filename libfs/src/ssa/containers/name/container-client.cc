...
NameContainer::VersionManager::vUpdate(SsaSession* session)
{
	...
	ssa::vm::client::VersionManager<NameContainer::Object>::vUpdate(session);
...
NameContainer::VersionManager::Erase(SsaSession* session, const char* name)
{
	...
	std::pair<EntryCache::iterator, bool> ret_pair = entries_.insert(std::pair<std::string, std::pair<bool, ssa::common::ObjectId> >(name, std::pair<bool, ssa::common::ObjectId>(false, oid)));
	neg_entries_count_++;
	assert(ret_pair.second == true);
