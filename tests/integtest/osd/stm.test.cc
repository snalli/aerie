...
SUITE(OSD_STM)
{
    ...
    TEST_FIXTURE(OsdFixture, TestPessimisticUpdate)
    {
        ...
        osd::client::rw::ObjectManager<NameContainer::Object, NameContainer::VersionManager>* mgr =
            new osd::client::rw::ObjectManager<NameContainer::Object,
                                               NameContainer::VersionManager>;
    ...
    TEST_FIXTURE(OsdFixture, TestOptimisticRead)
    {
        ...
        EVENT("AfterFind");
        CHECK(rw_reft->proxy()->Lock(session, lock_protocol::Mode::XL) ==
              E_SUCCESS); // hack to force the other client to publish the changes
        EVENT("AfterLock");
