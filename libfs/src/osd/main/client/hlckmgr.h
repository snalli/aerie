...
{
    HLockPtrLockModePair(HLock* hlock, lock_protocol::Mode mode) : hlock_(hlock), mode_(mode)
    {
    }
    HLock* hlock_;
...
class HLock
{
  public:
    ...
    LockId base_lid() const
    {
        return LockId(HLock::TypeId, lid_.number());
    }
...
class HLockManager : public LockRevoke
{
  ...
  public:
    ...
    void RegisterLockCallback(HLockCallback* hcb)
    {
        hcb_ = hcb;
    };
    void UnregisterLockCallback()
    {
        hcb_ = NULL;
    };
  ...
  private:
    ...
    lock_protocol::status AttachPublicLock(HLock* hlock, lock_protocol::Mode mode,
                                           bool caller_has_parent, int flags);
    lock_protocol::status AttachPublicLockChainUp(HLock* hlock, lock_protocol::Mode mode,
                                                  int flags);
    lock_protocol::status AttachPublicLockToChildren(HLock* hlock, lock_protocol::Mode mode);
    lock_protocol::status AttachPublicLockToChild(HLock* phlock, HLock* chlock,
                                                  lock_protocol::Mode mode);
    lock_protocol::status AttachPublicLockCapability(HLock* hlock, lock_protocol::Mode mode,
                                                     int flags);
    lock_protocol::status AcquireInternal(pthread_t tid, HLock* hlock, HLock* phlock,
    ...
    lock_protocol::status DowngradePublicLockRecursive(HLock* hlock,
...
inline int HLock::AddChild(HLock* hlock)
{
