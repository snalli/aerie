...
static int __ubench_hlock_create(osd::client::OsdSession* /*session*/, int numops, bool /*cache*/)
{
    ...
    {
        hlock[i] =
            global_storage_system->hlckmgr()->FindOrCreateLock(osd::cc::client::LockId(1, i));
    }
...
static int __ubench_hlock(osd::client::OsdSession* /*session*/, int numops, bool cache)
{
    MEASURE_TIME_PREAMBLE
    int ret = E_SUCCESS;
    unsigned long long runtime;
    ...
    if ((ret = global_storage_system->hlckmgr()->Acquire(osd::cc::client::LockId(1, 8),
                                                         lock_protocol::Mode::XR, 0)) != 0)
    {
        return ret;
    }
    ...
    if (cache)
    {
        for (int i = 0; i < numops; i++)
        {
            ...
            if ((ret = global_storage_system->hlckmgr()->Acquire(
                     osd::cc::client::LockId(1, i + 16), hlock, lock_protocol::Mode::XL, 0)) != 0)
            {
                return ret;
            }
            if ((ret = global_storage_system->hlckmgr()->Release(
                     osd::cc::client::LockId(1, i + 16))) < 0)
            {
                return ret;
            }
        }
...
static int __ubench_hlock_object(osd::client::OsdSession* /*session*/, int numops, bool cache)
{
    MEASURE_TIME_PREAMBLE
    int ret = E_SUCCESS;
    unsigned long long runtime;
    ...
    {
        hlock[i] =
            global_storage_system->hlckmgr()->FindOrCreateLock(osd::cc::client::LockId(1, i + 16));
    }
    ...
    if (cache)
    {
        ...
        {
            lock_protocol::Mode unused;
            if ((ret = global_storage_system->hlckmgr()->Acquire(hlock[i], phlock,
                                                                 lock_protocol::Mode::XL, 0)) != 0)
            {
                return ret;
            }
...
int ubench_hlock(int argc, char* argv[])
{
    extern int optind;
    extern int opterr;
    char ch;
...
int ubench_hlock_create(int argc, char* argv[])
{
    extern int optind;
    extern int opterr;
    char ch;
