...
int MapContainer::VersionManager::vOpen()
{
...
int MapContainer::VersionManager::vUpdate(OsdSession* session)
{
    osd::vm::client::VersionManager<MapContainer::Object>::vUpdate(session);
    return 0;
}

int MapContainer::VersionManager::Find(OsdSession* session, const char* key,
...
// already exists
int MapContainer::VersionManager::Insert(OsdSession* session, const char* key,
                                         osd::common::ObjectId oid)
{
    ...
    std::pair<EntryCache::iterator, bool> ret_pair =
        entries_.insert(std::pair<std::string, std::pair<bool, osd::common::ObjectId>>(
            key, std::pair<bool, osd::common::ObjectId>(true, oid)));
    assert(ret_pair.second == true);
    return E_SUCCESS;
}

int MapContainer::VersionManager::Erase(OsdSession* session, const char* key)
{
    EntryCache::iterator it;
    osd::common::ObjectId oid;
    int ret;
    ...
    std::pair<EntryCache::iterator, bool> ret_pair =
        entries_.insert(std::pair<std::string, std::pair<bool, osd::common::ObjectId>>(
            key, std::pair<bool, osd::common::ObjectId>(false, oid)));
    neg_entries_count_++;
    assert(ret_pair.second == true);
...
int MapContainer::VersionManager::Size(OsdSession* session)
{
