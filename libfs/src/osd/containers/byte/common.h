...
class ByteContainer
{
  public:
    ...
    template <typename Session> class Slot;

    template <typename Session> class Iterator;

    template <typename Session> class Region;

    template <typename Session> class Object : public osd::cc::common::Object
    {
      public:
        ...
        inline uint64_t get_maxbcount()
        {
            ...
            nblocks =
                radixtree_.height_ == 0 ? 0 : (1 << ((radixtree_.height_) * RADIX_TREE_MAP_SHIFT));
            nblocks += N_DIRECT;
    ...
    {
      public:
        Slot() : bcobj_(NULL), slot_base_(NULL)
        {
        }

        Slot(const Slot<Session>& copy)
            : bcobj_(copy.bcobj_), slot_base_(copy.slot_base_), slot_offset_(copy.slot_offset_),
              slot_height_(copy.slot_height_), base_bn_(copy.base_bn_)
        {
        }
        ...
        int Init(Session* /*session*/, Object<Session>* bcobj, void** slot_base, int slot_offset,
                 int slot_height)
        {
            bcobj_ = bcobj;
        ...
        int Init(Session* session, Object<Session>* const bcobj, uint64_t bn)
        {
            ...
            {
                rbn = bn - N_DIRECT;
                if ((ret = bcobj->radixtree_.MapSlot(session, rbn, 1, 0, &node, &slot_offset,
                                                     &slot_height)) != 0)
                {
                    slot_base_ = NULL;
                    return ret;
    ...
    {
      public:
        Region() : maxbcount_(0), base_bn_(0), size_(0)
        {
        }

        Region(const Region& copy)
            : slot_(copy.slot_), base_bn_(copy.base_bn_), maxbcount_(copy.maxbcount_),
              size_(copy.size_), dblock_(copy.dblock_), radixtree_(copy.radixtree_)
        {
        }
        ...
        Region(Session* session, Slot<Session>& slot) : slot_(slot)
        {
    ...
    {
      public:
        Iterator(Session* session = NULL) : session_(session)
        {
        }

        Iterator(Session* session, Object<Session>* bcobj, uint64_t bn = 0) : session_(session)
        {
...
inline uint64_t block_valid_data(uint64_t bn, uint64_t file_size)
{
    return ((((bn + 1) * kBlockSize) < file_size) ? kBlockSize : (file_size - (bn * kBlockSize)));
}
...
#endif
            return ((ret < 0) ? ((tot > 0) ? tot : ret) : tot + ret);
        }
    }
#ifdef DURABLE_DATA
    // wait for the writes to be performed to SCM
...
template <typename Session> ByteContainer::Object<Session>::Object() : size_(0)
{
...
void ByteContainer::Object<Session>::Free(Session* session, osd::common::ObjectId oid)
{
    ...
    for (iter = start, n = 0; !iter.terminate() && (uint64_t) n < obj->size_;
         iter++, n += kBlockSize)
    {
...
template <typename Session>
int ByteContainer::Object<Session>::InsertRegion(Session* session,
    ...
    else
    {
        ...
        else
        {
            ...
            if (radixtree_.rnode_)
            {
                ...
                // to the new one.
                assert(radixtree_.height_ > 0 && region->radixtree_.height_ > radixtree_.height_);
...
template <typename Session>
int ByteContainer::Object<Session>::ReadBlock(Session* session, char* dst, uint64_t bn, int off,
                                              int n)
{
    int l;
    int rn;
...
// Perhaps we should use some refcounting
template <typename Session>
int ByteContainer::Object<Session>::WriteBlock(Session* session, char* src, uint64_t bn, int off,
                                               int n)
{
    int ret;
...
template <typename Session>
int ByteContainer::Object<Session>::LinkBlock(Session* session, uint64_t bn, void* bp)
{
    int ret;
...
template <typename Session>
int ByteContainer::Object<Session>::Write(Session* session, char* src, uint64_t off, uint64_t n)
{
    return __Write<Session, ByteContainer::Object<Session>>(session, this, src, off, n);
}

template <typename Session>
int ByteContainer::Object<Session>::ReadImmutable(Session* session, char* dst, uint64_t off,
                                                  uint64_t n)
{
    uint64_t tot;
    uint64_t m;
    ...
    // off+n-1, n); printf ("Immutable range = [%" PRIu64 ", %" PRIu64 "] n=%" PRIu64 "\n", off,
    // off+n-1, n);
    ...
    for (tot = 0, bn = fbn; !iter.terminate() && tot < n; iter++, tot += m, off += m, bn++)
    {
        bcount = 1 << (((*iter).slot_height_ - 1) * RADIX_TREE_MAP_SHIFT);
        size = bcount * kBlockSize;
        m = min(n - tot, size - f);
...
template <typename Session>
int ByteContainer::Object<Session>::Read(Session* session, char* dst, uint64_t off, uint64_t n)
{
    return ReadImmutable(session, dst, off, n);
}

template <typename Session> void ByteContainer::Object<Session>::PrintBlocks(Session* session)
{
    int ret;
    void** slot;
    RadixTreeNode<Session>* node;
...
template <typename Session>
int ByteContainer::Object<Session>::LookupSlot(Session* session, uint64_t bn, Slot<Session>* slot)
{
    if (!slot)
    {
        return -E_INVAL;
 ...
 */
template <typename Session>
int ByteContainer::Object<Session>::LowerBound(Session* session, uint64_t bn, uint64_t* lge_bn)
{
...
template <typename Session>
int ByteContainer::Region<Session>::Extend(Session* session, uint64_t bn)
{
 ...
 */
template <typename Session>
void** ByteContainer::Region<Session>::MapSlot(Session* session, uint64_t bn, bool alloc_slot)
{
    int ret;
    void** slot;
    RadixTreeNode<Session>* node;
...
template <typename Session>
int ByteContainer::Region<Session>::WriteBlock(Session* session, char* src, uint64_t bn, int off,
                                               int n)
{
    void** slot;
    char* bp;
    ...
    {
        void* ptr;
        if ((ret = session->salloc()->AllocateExtent(session, kBlockSize, kData, &ptr)) < 0)
        {
            return ret;
        }
        if (session->journal()->mode() == osd::common::Journal::Client)
        {
            session->journal() << osd::Publisher::Message::ContainerOperation::LinkBlock(
                slot_.bcobj_->oid(), bn, ptr);
        }
...
// off is offset with respect to the block
template <typename Session>
int ByteContainer::Region<Session>::LinkBlock(Session* session, uint64_t bn, void* bp)
{
    void** slot;
...
template <typename Session>
int ByteContainer::Region<Session>::Write(Session* session, char* src, uint64_t off, uint64_t n)
{
    return __Write<Session, ByteContainer::Region<Session>>(session, this, src, off, n);
}

template <typename Session>
int ByteContainer::Region<Session>::ReadBlock(Session* session, char* dst, uint64_t bn, int off,
                                              int n)
{
    void** slot;
    char* bp;
...
template <typename Session>
int ByteContainer::Region<Session>::Read(Session* session, char* dst, uint64_t off, uint64_t n)
{
