...
static int test_large_value()
{
    ...
    int rc = 0;
    if (kvfs_put(key, val, size) != size)   { rc = -1; goto done; }
    if (kvfs_get(key, buf)       != size)   { rc = -1; goto done; }
    if (memcmp(buf, val, size)   != 0)      { rc = -1; goto done; }
...
int main(int argc, char* argv[])
{
    ...
    if (ret < 0) { fprintf(stderr, "kvfs_init2 failed: %d\n", ret); return 1; }
    ret = kvfs_mount("/tmp/stamnos_pool", 0);
    if (ret < 0) { fprintf(stderr, "kvfs_mount failed: %d\n", ret); return 1; }
