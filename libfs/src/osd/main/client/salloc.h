...
class DescriptorPool
{
  public:
    DescriptorPool(int capability, osd::common::ObjectId oid)
        : capability_(capability), last_offset_(0)
    {
        set_obj_ = ObjectIdSet::Load(oid);
    }

    static int Create(::client::Ipc* ipc, OsdSession* session, osd::common::AclIdentifier acl_id,
                      DescriptorPool** poolp);

    int LoadFromLast(OsdSession* session);
    int Load(OsdSession* session);
    int AllocateContainer(::client::Ipc* ipc, OsdSession* session, int type,
                          osd::common::ObjectId* oid);
    int AllocateExtent(::client::Ipc* ipc, OsdSession* session, size_t nbytes,
...
{
    typedef google::dense_hash_set<::osd::common::ObjectId, ::osd::common::ObjectIdHashFcn>
        ObjectIdSet;
    typedef google::dense_hash_map<::osd::common::AclIdentifier, DescriptorPool*> AclPoolMap;

  public:
    StorageAllocator(::client::Ipc* ipc) : ipc_(ipc)
    {
    ...
    int AllocateExtent(OsdSession* session, osd::common::AclIdentifier acl_id, size_t nbytes,
                       int flags, void** ptr);
    int AllocateExtent(OsdSession* session, osd::common::AclIdentifier acl_id, size_t nbytes,
                       osd::common::ExtentId* eidp, bool has_lock = false);
