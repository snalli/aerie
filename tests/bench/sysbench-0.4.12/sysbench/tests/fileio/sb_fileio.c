...
static sb_arg_t fileio_args[] = {
  ...
  {"file-fs-type", "file system type {local, lib}", SB_ARG_TYPE_STRING, "local"},
...
int file_init(void)
{
  char* argv[] = {"test","-h 10000"};
  if (parse_arguments())
    return 1;
  
  if (file_fs_type == FS_TYPE_LIBRARY) {	
    libfs_init(2, argv);
...
#ifndef _WIN32
    if (file_fs_type == FS_TYPE_LIBRARY) {	
      files[i] = libfs_open2(file_name, O_CREAT | O_RDWR | file_extra_flags,
                      S_IRUSR | S_IWUSR);
    } else {
                    ...
                    long long offset, int thread_id)
{
  ...
  if (file_io_mode == FILE_IO_MODE_SYNC)
    if (file_fs_type == FS_TYPE_LIBRARY) {	
      return libfs_pwrite(fd, buf, count, offset);
	} else {
...
int parse_arguments(void)
{
  ...
  fs_type = sb_get_value_string("file-fs-type");
  if (fs_type == NULL) {
    file_fs_type = FS_TYPE_LOCAL;
  } else {
    if (!strcmp(fs_type, "lib"))
      file_fs_type = FS_TYPE_LIBRARY;
    else if (!strcmp(fs_type, "local"))
      file_fs_type = FS_TYPE_LOCAL;
    else
    {
      log_text(LOG_FATAL, "Invalid file system type: %s.", fs_type);
      return 1;
    }
