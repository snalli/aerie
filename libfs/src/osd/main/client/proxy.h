...
{
    // insight : Notice that the name of the child and parent class are same simply because they are
    // in different namespaces
  public:
    ObjectProxy(OsdSession* session, osd::common::ObjectId oid) : osd::common::ObjectProxy(oid)
    {
    ...
    void JournalLockCertificate(OsdSession* session)
    {
        ...
        Publisher::Message::ContainerOperation::LockCertificate* certificate =
            new (buf) Publisher::Message::ContainerOperation::LockCertificate(nlocks);

        assert(sizeof(*certificate) + certificate->payload_size_ <
               128); // otherwise we write outside buffer buf
        if (nlocks > 0)
        {
...
template <class Derived, class Subject> class ObjectProxyTemplate : public ObjectProxy
{
  public:
    ObjectProxyTemplate(OsdSession* session, osd::common::ObjectId oid) : ObjectProxy(session, oid)
    { // printf("\nVerify 4. In osd/main/client/proxy.h");}
    }

    Subject* object()
    {
        return static_cast<Subject*>(object_);
    }

    Derived* xOpenRO(osd::stm::client::Transaction* tx);
    Derived* xOpenRO();
    // ~ObjectProxyTemplate() { printf("\n + Destroying ObjectProxy 002. in
    // src/osd/main/client/proxy.h");}
...
template <class Derived, class Subject> Derived* ObjectProxyTemplate<Derived, Subject>::xOpenRO()
{
...
{
  public:
    VersionManager(Subject* object = NULL) : object_(object), nlink_(0)
    { // printf("\n-------> Ola Mofus !");}
    }
...
class ObjectProxy : public osd::cc::client::ObjectProxyTemplate<Derived, Subject>
{
  public:
    ObjectProxy(OsdSession* session, osd::common::ObjectId oid)
        : osd::cc::client::ObjectProxyTemplate<Derived, Subject>(session, oid), valid_(false)
    {
        // CAREFUL:
    ...
    // ~ObjectProxy() { printf("\n + Sanketh : Destroying ObjectProxy 003. in
    // src/osd/main/client/proxy.h");}
