...
template <int size> inline slab<size>::slab(int fs)
{
    tail = NULL;
    slices = NULL;
    fragment_size = fs;
...
template <int size> inline slab<size>::~slab()
{
    shutdown();
}

template <int size> inline void slab<size>::shutdown()
{
    slab_free_list *tmp, *tmp2;
    int cells = 0;

    for (tmp = tail; tmp; tmp = tmp->n)
        cells++;
    for (tmp = slices; tmp;)
    {
...
template <int size> inline void* slab<size>::alloc()
{
    void* slot;
...
template <int size> inline void slab<size>::release(void* ptr)
{
