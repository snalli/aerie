...
NameSpace::NameSpace(const char* namespace_name)
{
    ...
    /*		dentries_.set_empty_key("__EMPTY_KEY__");
       ...
       \nCache Accs : %d \n",getpid(), cacheHit, cacheMiss, cacheLookup); printf("Cache Inst :
       %d\n", succEntry); printf("\n***************************************************\n");
    */
...
int NameSpace::Init(Session* /*session*/)
{
    root_ = new MPInode;
...
#ifdef CONFIG_CACHE
void NameSpace::SetCache(osd::common::Cache* val)
{
    cache = val;
}
int NameSpace::Lookup(const char* name, void** ip, void** dip)
{
...
int NameSpace::Insert(const char* name, void* ip, void* dip)
{
    int index = strlen(name) % N_CACHE;
    return cache_arr[index]->insert_cache(name, ip, dip);
}
int NameSpace::Erase(const char* name, void* /*ip*/, void* /*dip*/)
{
    int index = strlen(name) % N_CACHE;

    return cache_arr[index]->erase_cache(name);
}
void NameSpace::Prefetch(Session* session)
{
...
int NameSpace::Mount(Session* session, const char* const_path, SuperBlock* sb)
{
    char name[128];
    Inode* inode;
    Inode* inode_next;
...
int NameSpace::Unmount(Session* /*session*/, char* /*name*/)
{
    return 0;
}
...
int NameSpace::LockInodeReverse(Session* session, Inode* inode, lock_protocol::Mode lock_mode)
{
    ...
    inode_chain.clear();
    for (std::vector<Inode*>::iterator it = locked_inodes.begin(); it != locked_inodes.end(); it++)
    {
        (*it)->Unlock(session);
    }
    locked_inodes.clear();
    ...
    parent_inode = NULL;
    for (std::vector<Inode*>::iterator it = inode_chain.end() - 1; it >= inode_chain.begin(); it--)
    {
        if (*it == inode)
        {
            if (parent_inode)
...
//
int NameSpace::Namex(Session* session, const char* cpath, lock_protocol::Mode lock_mode,
...
traditional:
    if (*path == '/')
    {
        inode = root_; // The root inode, as always, is "well known" !
                       ...
                       // You cannot instantiate an abstract class, because it has virtual functions
                       // which have not yet been defined. The task of defining the virtual
                       // functions is delegated to child classes. In this case, Inode::Lock is a
                       // virtual function which needs to be defined before use. MPInode does this,
                       // and hence we are able to use Inode::Lock.
 ...
 */
        ...
        { // If we are here, it means the filename was just "/" !!!!!!
    ...
    else
    {
        ...
        else
        {
            if (inode->Lock(session, lock_mode) != 0)
            {
                LockInodeReverse(
                    session, inode,
                    lock_protocol::Mode::IXSL); // insight : Locking all the ancestors in SIX mode
            }
            inode->Get();
        }
    }
...
resume_normal:
    while (path != 0)
    { // Traverse the pathname to get to target file
        // DO ME : Before the inode lookup, you can have a cache look up !
        // If that fails, then you can proceed with the inode lookup
        /*
                        s_log("[%ld] NameSpace::%s ======================================",s_tid,
           __func__); s_log("[%ld] NameSpace::%s itr=%d",s_tid, __func__, itr++); s_log("[%ld]
           NameSpace::%s name=%s",s_tid, __func__, name); s_log("[%ld] NameSpace::%s
           dup_path=%s",s_tid, __func__, dup_path); s_log("[%ld] NameSpace::%s path=%s",s_tid,
           __func__, path); s_log("[%ld] NameSpace::%s inode=%p",s_tid, __func__, inode);
                        s_log("[%ld] NameSpace::%s ======================================",s_tid,
           __func__);
        */
...
int NameSpace::namex_sans_locks(Session* session, const char* cpath, lock_protocol::Mode lock_mode,
                                bool nameiparent, char* name, Inode** inodep)
{
    char* path;
    Inode* inode;
...
int NameSpace::Nameiparent(Session* session, const char* path, lock_protocol::Mode lock_mode,
...
int NameSpace::Namei(Session* session, const char* path, lock_protocol::Mode lock_mode,
                     Inode** inodep)
{
    int ret;
    char name[128];
...
int NameSpace::namei_sans_locks(Session* session, const char* path, lock_protocol::Mode lock_mode,
                                Inode** inodep)
{
    int ret;
    char name[128];
...
int NameSpace::SetCurWrkDir(Session* session, const char* path)
{
    Inode* inode;
    int ret;
...
int NameSpace::Rename(Session* session, const char* oldpath, const char* newpath)
{
    char name_old[128];
    char name_new[128];
    Inode* dp_old;
...
grab_locks:
    ...
    if ((ret = global_namespace->Nameiparent(session, oldpath, lock_protocol::Mode::XR, name_old,
                                             &dp_old)) < 0)

    {
        return ret;
    }
    if ((ret = global_namespace->Nameiparent(session, newpath, lock_protocol::Mode::XR, name_new,
                                             &dp_new)) < 0)
    {
        dp_old->Put();
    ...
    session->journal() << Publisher::Message::LogicalOperation::Link(dp_new->ino(), name_new,
                                                                     ip->ino());
...
int NameSpace::Link(Session* session, const char* oldpath, const char* newpath)
{
    char name[128];
    Inode* dp;
    Inode* ip;
    ...
    if ((ret = global_namespace->Namei(session, oldpath, lock_protocol::Mode::XL, &ip)) < 0)
    {
        return ret;
    }
    ...
    if ((ret = global_namespace->Nameiparent(session, newpath, lock_protocol::Mode::XL, name,
                                             &dp)) < 0)
    {
        goto bad;
    }
...
int NameSpace::Unlink(Session* session, const char* pathname)
{
    ...
    */
    ...
    // on its child
    if ((ret = global_namespace->Nameiparent(session, pathname, lock_protocol::Mode::XL, name,
                                             &dp)) < 0)
    {
        return ret;
    }
