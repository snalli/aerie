...
int Client::Mount(const char* source, uint32_t flags)
{
    int ret;
    ...
    if ((ret = global_ipc_layer->call(FileSystemProtocol::kMount, global_ipc_layer->id(),
                                      std::string(source), flags, mntrep)) < 0)
    {
        return -E_IPC;
    }
    if (ret > 0)
...
int Client::Put(const char* key, const char* src, uint64_t n)
{
    return tp_->Put(CurrentSession(), key, src, n);
}

int Client::Get(const char* key, char* dst)
{
    int ret;
