...
int InodeFactory::LoadDirInode(::client::Session* session, ::osd::common::ObjectId oid,
                               ::client::Inode** ipp)
{
...
int InodeFactory::MakeDirInode(::client::Session* session, ::client::Inode** ipp)
{
    int ret = E_SUCCESS;
...
int InodeFactory::LoadFileInode(::client::Session* session, ::osd::common::ObjectId oid,
                                ::client::Inode** ipp)
{
    osd::common::ObjectProxyReference* ref;
    FileInode* fip;
    ...
    *ipp = fip;
    ...
    // insight : Looks like, we have two branches that create file inode and fill the address to it
    in *ipp.
    // insight : The difference the two branches is something to do with locks.
    // insight : KNOW ME : Currently, I theorise that there is a pool of inode objects and if the
    the object we want exist, simply reutrn it else create it.

        if ((ret = session->omgr_->FindObject(session, oid, &ref)) == E_SUCCESS) {
        // insight : The question to ponder here is this. LoadFileInode does not know the origin of
    oid Object.
        // The oid object could have been created right before the call to LoadFileInode or is an
    existent one.
        // There are two sources of origin : Existent and Created. Can it happen that an oid that
    was created have an
        // objproxy_ref in the hash table but no inode ? If not we can avoid the call to FindObject.
            ...
            if (ref->owner()) {
                // the in-core inode already exists; just return this and
                // we are done
                fip = reinterpret_cast<FileInode*>(ref->owner()); // insight : This is not where we
    create the inode } else { if (!wrlock) { pthread_rwlock_unlock(&rwlock_);
                    pthread_rwlock_wrlock(&rwlock_);
                }

                if (ref->owner()) {
                    fip = reinterpret_cast<FileInode*>(ref->owner());// insight : This is not where
    we create the inode
...
int InodeFactory::MakeFileInode(::client::Session* session, ::client::Inode** ipp)
{
    int ret = E_SUCCESS;
    ...
    if ((obj = osd::containers::client::ByteContainer::Object::Make(session)) == NULL)
    {
        ...
        // insight : KNOW ME : Inode creation not fully understood. Need to look at session
        // session->salloc()->AllocateContainer
        return -E_NOMEM;
    }
...
int InodeFactory::DestroyFileInode(::client::Session* session, ::client::Inode* ip)
{
    int ret = E_SUCCESS;
    osd::common::ObjectProxyReference* ref;
...
int InodeFactory::MakeInode(::client::Session* session, int type, ::client::Inode** ipp)
{
    int ret = E_SUCCESS;
...
int InodeFactory::LoadInode(::client::Session* session, osd::common::ObjectId oid,
                            ::client::Inode** ipp)
{
    int ret = E_SUCCESS;
    ...
    case osd::containers::T_NAME_CONTAINER: // directory
        ...
        break;
    case osd::containers::T_BYTE_CONTAINER: // insight : Fancy names. Clearly, T_*_CONTAINER is an
                                            // integer because it is in a switch case.
...
int InodeFactory::DestroyInode(::client::Session* session, ::client::Inode* ip)
{
    int ret = E_SUCCESS;
...
int InodeFactory::Make(::client::Session* session, int type, ::client::Inode** ipp)
{
    return MakeInode(session, type, ipp);
}

int InodeFactory::Load(::client::Session* session, osd::common::ObjectId oid, ::client::Inode** ipp)
{
    return LoadInode(session, oid, ipp);
}

int InodeFactory::Destroy(::client::Session* session, ::client::Inode* ip)
{
