...
SUITE(OSD_Object)
{
    TEST_FIXTURE(OsdFixture, Test)
    {
        ...
        osd::client::rw::ObjectManager<Dummy, DummyVersionManager>* dummy_mgr =
            new osd::client::rw::ObjectManager<Dummy, DummyVersionManager>;
    ...
    TEST_FIXTURE(OsdFixture, TestAlloc)
    {
        ...
        CHECK(global_storage_system->salloc()->AllocateContainer(
                  session, 0, osd::containers::T_NAME_CONTAINER, &oid) == E_SUCCESS);
        CHECK(global_storage_system->salloc()->AllocateContainer(
                  session, 0, osd::containers::T_NAME_CONTAINER, &oid) == E_SUCCESS);
        CHECK(global_storage_system->salloc()->AllocateContainer(
                  session, 0, osd::containers::T_BYTE_CONTAINER, &oid) == E_SUCCESS);
        CHECK(global_storage_system->salloc()->AllocateContainer(
                  session, 0, osd::containers::T_BYTE_CONTAINER, &oid) == E_SUCCESS);

        CHECK(global_storage_system->salloc()->AllocateContainer(
                  session, 1, osd::containers::T_BYTE_CONTAINER, &oid) == E_SUCCESS);
        CHECK(global_storage_system->salloc()->AllocateContainer(
                  session, 1, osd::containers::T_BYTE_CONTAINER, &oid) == E_SUCCESS);
    }
}
