...
struct RunTestIfPrefixMatchFilter
{
    RunTestIfPrefixMatchFilter(char const* suite_filter, char const* test_filter)
        : suite_filter_(suite_filter), test_filter_(test_filter)
    {
    }
...
inline Test::Test(ArgValMap* global, ArgValMap* local)
    : msgid_(-1), semid_(-1), filter_type_(NULL), suite_name_(NULL), test_name_(NULL),
      tag_name_(NULL), oss_(std::ostringstream::out)
{
    std::string* val;
...
inline void* Test::ThreadEntryPoint(void* arg)
{
    int ret;
    Test* t = (Test*) arg;
...
inline int Test::Event(const char* event)
{
    char buf[512];
...
inline int TestFramework::ParseArg(int argc, char** argv)
{
    char ch;
...
inline TestFramework::TestFramework(int argc, char** argv)
    : argc_(argc), argv_(argv), argvmap_global_(NULL)
{
    int nthreads = 0;

    pthread_mutex_init(&mutex_, NULL);
    ParseArg(argc, argv);
    for (std::set<ArgValMap*>::iterator it = argvmap_set_.begin(); it != argvmap_set_.end(); ++it)
    {
    ...
    if (nthreads > 0)
    {
        for (std::set<ArgValMap*>::iterator it = argvmap_set_.begin(); it != argvmap_set_.end();
             ++it)
        {
            ArgValMap* argvmap = *it;
            if (argvmap->ArgVal(std::string("thread")))
...
inline int TestFramework::RunTests()
{
    int test_failed_count = 0;

    numthreads_ = test_set_.size();
    for (std::set<Test*>::iterator it = test_set_.begin(); it != test_set_.end(); ++it)
    {
...
inline Test* TestFramework::tid2Test(pthread_t tid)
{
    for (std::set<Test*>::iterator it = test_set_.begin(); it != test_set_.end(); ++it)
    {
        Test* t = *it;
...
inline int TestFramework::ArgVal(std::string& key, std::string& val)
{
    if (argvmap_global_)
    {
...
inline int TestFramework::ArgVal(const char* key, std::string& val)
{
    std::string keystr(key);
    return ArgVal(keystr, val);
}

inline void TestFramework::RegisterFinalize(AbstractFunctor* functor)
{
