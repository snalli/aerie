...
int SetContainer<T>::Object<Session>::Insert(Session* session, T val)
{
    uint64_t size = size_++;
    char* c = (char*) &val;
...
template <typename T>
template <typename Session>
int SetContainer<T>::Object<Session>::Read(Session* session, int pos, T* val)
{
    char c[sizeof(T)];
...
template <typename T>
template <typename Session>
int SetContainer<T>::Object<Session>::Write(Session* session, int pos, T val)
{
    char c[8];
...
template <typename T> template <typename Session> int SetContainer<T>::Object<Session>::Size()
{
