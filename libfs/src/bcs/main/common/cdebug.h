...
#define ACTION(name) dbg_module_##name,

    enum
    {
        FOREACH_DEBUG_MODULE(ACTION) dbg_module_count
    };
#undef ACTION
...
#define DBG_MODULE(name) dbg_module_##name
    ...
    static char* dbg_code2str[] = {
        (char*) "Off",     (char*) "CRITICAL", (char*) "ERROR",
        (char*) "WARNING", (char*) "INFO",     (char*) "DEBUG",
    };
...
#define dbg_log(level, format, ...)                                                                \
    ...
    {                                                                                              \
        FILE* ferr = stdout;                                                                       \
        if (level && (level <= dbg_level || level <= dbg_terminate_level))                         \
        {                                                                                          \
            if (level <= dbg_stderr_level)                                                         \
            {                                                                                      \
                ferr = stderr;                                                                     \
            }                                                                                      \
            fprintf(ferr, "[%s] %s in %s <%s,%d>: " format, dbg_identifier, dbg_code2str[level],   \
