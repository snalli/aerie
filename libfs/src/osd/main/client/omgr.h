...
class ObjectManagerOfType
{
  ...
  public:
    virtual ObjectProxy* Load(OsdSession* session,
                              ObjectId oid) = 0; // insight : oid gets used here !!
...
class ObjectManager : public osd::cc::client::HLockCallback, public osd::cc::client::LockCallback
{
  ...
  public:
    ...
    int FindOrGetObject(OsdSession* session, ObjectId oid,
  ...
  private:
    int GetObjectInternal(OsdSession* session, ObjectId oid,
                          osd::common::ObjectProxyReference** obj_ref, bool use_exist_obj_ref);
