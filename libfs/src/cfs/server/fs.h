...
class FileSystem
{
  public:
    ...
    int Init();
    int MakeFile(Session* session, const char* path, unsigned int flags, unsigned int mode,
                 FileSystemProtocol::InodeNumber& ino);
    int MakeDir(Session* session, const char* path, unsigned int mode,
                FileSystemProtocol::InodeNumber& ino);
    int Unlink(Session* session, const char* path);
    int Read(Session* session, FileSystemProtocol::InodeNumber ino, void* buf, int count,
             int offset, int& n);
    int Write(Session* session, FileSystemProtocol::InodeNumber ino, void* buf, int count,
              int offset, int& n);
    ...
    int Mount(int clt, const char* source, const char* target, unsigned int flags,
              FileSystemProtocol::MountReply& rep);

    StorageSystem* storage_system()
    {
        return storage_system_;
    }
    ...
    class IpcHandlers
    {
      public:
        ...
        int Mount(unsigned int clt, std::string source, std::string target, unsigned int flags,
                  FileSystemProtocol::MountReply& rep);
        int MakeFile(unsigned int clt, std::string path, unsigned int flags, unsigned int mode,
                     FileSystemProtocol::InodeNumber& ino);
        int MakeDir(unsigned int clt, std::string path, unsigned int mode,
                    FileSystemProtocol::InodeNumber& ino);
        int Read(unsigned int clt, FileSystemProtocol::InodeNumber ino, int count, int offset,
                 int& r);
        int Write(unsigned int clt, FileSystemProtocol::InodeNumber ino, int count, int offset,
                  int& r);
