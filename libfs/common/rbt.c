...
err_alloc_newTree_root:
  free(newTree->nil);
err_alloc_newTree_nil:
  free(newTree);
}
...
void InorderTreePrint(rb_red_blk_tree* tree, rb_red_blk_node* x) {
  ...
  if (x != tree->nil) {
    ...
    printf("info=");
    if (tree->PrintInfo) tree->PrintInfo(x->info);
    printf("  key="); 
	if (tree->PrintKey) {
      tree->PrintKey(x->key);
	}   
    printf("  l->key=");
    if( x->left == nil) printf("NULL"); else {
	  if (tree->PrintKey) tree->PrintKey(x->left->key);	
	}  
    printf("  r->key=");
    if( x->right == nil) printf("NULL"); else {
	  if (tree->PrintKey) tree->PrintKey(x->right->key);	
    }
    printf("  p->key=");
    if( x->parent == root) printf("NULL"); else {
	  if (tree->PrintKey) tree->PrintKey(x->parent->key);	
    }
    printf("  red=%i\n",x->red);
...
/***********************************************************************/
/*  FUNCTION:  RBQueryLargestSmaller */
/**/
/*    INPUTS:  tree is the tree to print and q is a pointer to the key */
/*             we are searching for */
/**/
/*    OUTPUT:  returns the a node with the largest key which is smaller than q.  
/**/
/*    Modifies Input: none */
/**/
/***********************************************************************/
  
rb_red_blk_node* RBQueryLargestSmaller(rb_red_blk_tree* tree, void* q) {
  rb_red_blk_node* x=tree->root->left;
  rb_red_blk_node* nil=tree->nil;
  int compVal;
  if (x == nil) return(0);
...
int InorderTreeTraversal(rb_red_blk_tree* tree, rb_red_blk_node* x, 
                          ...
                          void* arg, void* retv) 
{
  ...
  rb_red_blk_node* root=tree->root;
  if (x != tree->nil) {
    if (InorderTreeTraversal(tree,x->left, callback, arg, retv) == 0) {
		return 0;
	}
	if (callback(x->key, x->info, arg, retv) == 0) {
		// short circuit 
		return 0;
	}
    if (InorderTreeTraversal(tree,x->right, callback, arg, retv) == 0) {
		return 0;
	}
  }
  return -1;
...
int RBTreeTraversal(rb_red_blk_tree* tree, 
                    int (*callback)(void*, void*, void*, void*),
                    void* arg, void* retv) 
{
  return InorderTreeTraversal(tree,tree->root->left, callback, arg, retv);
}
