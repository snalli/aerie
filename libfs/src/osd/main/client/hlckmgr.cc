...
HLock::HLock(LockId lid)
    : lock_(NULL), parent_(NULL), sticky_(false), used_(false), can_retry_(false),
      mode_(lock_protocol::Mode(lock_protocol::Mode::NL)),
      ancestor_recursive_mode_(lock_protocol::Mode(lock_protocol::Mode::NL)), lid_(lid),
      owner_depth_(0), status_(NONE)
{
    // insight : Constructor
...
void HLock::set_status(LockStatus sts)
{
    DBG_LOG(DBG_INFO, DBG_MODULE(client_hlckmgr), "set_status lock %s: %d --> %d\n", lid_.c_str(),
            status_, sts);
    if (status_ != sts)
    {
        if (sts == LOCKED)
...
int HLock::BeginConverting(bool lock)
{
    if (lock)
    {
        pthread_mutex_lock(&mutex_);
    }
    while (!(status_ == HLock::FREE || status_ == HLock::LOCKED))
    {
...
int HLock::EndConverting(bool lock)
{
    if (lock)
    {
        pthread_mutex_lock(&mutex_);
...
// to new_status
int HLock::ChangeStatus(LockStatus old_sts, LockStatus new_sts)
{
    pthread_mutex_lock(&mutex_);
...
int HLock::WaitStatus2(LockStatus old_sts1, LockStatus old_sts2)
{
...
//
// assumes mutex lock is help
int HLock::WaitStatus(LockStatus old_sts)
{
    while (!(status_ == old_sts))
    {
        pthread_cond_wait(&status_cv_, &mutex_);
...
HLockManager::HLockManager(LockManager* lm) : status_(NONE), hcb_(NULL), lm_(lm)
{
...
{
    DBG_LOG(DBG_INFO, DBG_MODULE(client_hlckmgr), "[%d] Shutting down Hierarchical Lock Manager\n",
            id());
...
HLock* HLockManager::FindLockInternal(LockId lid)
{
    HLock* lp;
...
HLock* HLockManager::FindOrCreateLockInternal(LockId lid)
{
    HLock* lp;
    ...
    {
        DBG_LOG(DBG_INFO, DBG_MODULE(client_hlckmgr), "[%d] Creating hierarchical lock %s\n", id(),
                lid.c_str());
        lp = new HLock(lid);
        locks_[lid] = lp;
    }
 ...
 */
HLock* HLockManager::FindOrCreateLock(LockId lid)
{
    HLock* hlock;
...
lock_protocol::status HLockManager::AttachPublicLock(HLock* hlock, lock_protocol::Mode mode,
...
///
lock_protocol::status HLockManager::AttachPublicLockChainUp(HLock* hlock, lock_protocol::Mode mode,
                                                            int /*flags*/)
{
    struct LockDsc
    {
    ...
    hl_public = hl;
    (void) hl_public; // only read in assert; suppress unused warning in release
    ...
    for (hlv_itr = hlv.end() - 1; hlv_itr >= hlv.begin(); hlv_itr--)
    {
        ...
        assert(lock_protocol::Mode::AbidesRecursiveRule(hl_public->lock_->public_mode_, hl->mode_));
...
lock_protocol::status HLockManager::AttachPublicLockToChild(HLock* phlock, HLock* chlock,
                                                            lock_protocol::Mode mode)
{
    ...
    DBG_LOG(DBG_INFO, DBG_MODULE(client_hlckmgr),
            ...
            mode.String().c_str(), chlock->lid_.c_str(), phlock->lid_.c_str());
    ...
    {
        if ((r = AttachPublicLock(chlock, mode, true, HLock::FLG_NOBLK)) == lock_protocol::RETRY)
        {
...
lock_protocol::status HLockManager::AttachPublicLockToChildren(HLock* hlock,
                                                               lock_protocol::Mode mode)
{
    ...
    DBG_LOG(DBG_INFO, DBG_MODULE(client_hlckmgr),
            ...
            mode.String().c_str(), hlock->lid_.c_str());
...
lock_protocol::status HLockManager::AcquireInternal(pthread_t tid, HLock* hlock, HLock* phlock,
                                                    lock_protocol::Mode mode, int flags)
{
    ...
    DBG_LOG(DBG_INFO, DBG_MODULE(client_hlckmgr),
            ...
            lid.string().c_str(), mode.String().c_str(),
...
check_state:
    ...
    case HLock::FREE:
        ...
        if (hlock->lock_)
        {
            ...
            DBG_LOG(DBG_INFO, DBG_MODULE(client_hlckmgr),
                    "[%d:%lu] trying to acquire locally free hierarchical lock %s at mode %s (base "
                    "lock mode %s, hlock mode %s): "
                    "grant to thread %lu\n",
                    id(), tid, lid.c_str(), mode.String().c_str(),
                    hlock->lock_->public_mode_.String().c_str(), hlock->mode_.String().c_str(),
                    tid);
            ...
            else
            {
                ...
                else
                {
                    ...
                    DBG_LOG(DBG_CRITICAL, DBG_MODULE(client_hlckmgr), "Must convert public lock\n");
                }
            }
        }
        else
        {
            ...
            if (hlock->parent_)
            {
                ...
                if (lock_protocol::Mode::AbidesRecursiveRule(mode, hlock->ancestor_recursive_mode_))
                {

                    hlock->mode_ = lock_protocol::Mode::Supremum(hlock->mode_, mode);

                    assert(lock_protocol::Mode::AbidesRecursiveRule(
                        hlock->mode_, hlock->ancestor_recursive_mode_));
                    hlock->owner_ = tid;
                    hlock->owner_depth_ = 1;
                    hlock->set_status(HLock::LOCKED);
                    r = lock_protocol::OK;
            ...
            {
                DBG_LOG(DBG_CRITICAL, DBG_MODULE(client_hlckmgr), "No base lock and no parent\n");
            }
        }
        break;
    ...
    case HLock::CONVERTING:
        ...
        while (hlock->status() == HLock::CONVERTING || hlock->status() == HLock::LOCKED_CONVERTING)
        {
        ...
        if (phlock)
        {
            ...
            else
            {
                ...
                DBG_LOG(DBG_INFO, DBG_MODULE(client_lckmgr),
                        ...
                        tid, lid.c_str(), phlock->lid_.c_str());
                phlock->AddChild(hlock);
...
lock_protocol::status HLockManager::Acquire(HLock* hlock, lock_protocol::Mode mode, int flags)
{
    lock_protocol::status r;
...
lock_protocol::status HLockManager::Acquire(HLock* hlock, HLock* phlock, lock_protocol::Mode mode,
                                            int flags)
{
    lock_protocol::status r;
...
lock_protocol::status HLockManager::Acquire(LockId lid, LockId plid, lock_protocol::Mode mode,
                                            int flags)
{
    lock_protocol::status r;
    HLock* hlock;
...
lock_protocol::status HLockManager::Acquire(LockId lid, HLock* phlock, lock_protocol::Mode mode,
                                            int flags)
{
    lock_protocol::status r;
    HLock* hlock;
...
lock_protocol::status HLockManager::Acquire(LockId lid, lock_protocol::Mode mode, int flags)
{
    lock_protocol::status r;
    HLock* hlock;
 ...
 */
lock_protocol::status HLockManager::ReleaseInternal(pthread_t tid, HLock* hlock, bool /*force*/)
{
    lock_protocol::status r = lock_protocol::OK;
    LockId lid = hlock->lid_;
    (void) lid;

    DBG_LOG(DBG_INFO, DBG_MODULE(client_hlckmgr), "[%d:%lu] Releasing hierarchical lock %s\n", id(),
            tid, lid.c_str());
...
lock_protocol::status HLockManager::Release(HLock* hlock)
{
    lock_protocol::status r;
...
lock_protocol::status HLockManager::Release(LockId lid)
{
    lock_protocol::status r;
    HLock* hlock;
    ...
    if (hlock == NULL)
    {
        DBG_LOG(DBG_WARNING, DBG_MODULE(client_hlckmgr), "Unknown hierarchical lock %s\n",
                lid.c_str());
        return lock_protocol::NOENT;
    }
    pthread_mutex_lock(&hlock->mutex_);
 ...
 */
lock_protocol::status
HLockManager::DowngradePublicLockRecursive(HLock* hlock, lock_protocol::Mode new_public_mode,
                                           HLockPtrLockModePairSet* release_set)
{
    HLockPtrList::iterator itr;
    lock_protocol::Mode old_public_mode;

    DBG_LOG(DBG_INFO, DBG_MODULE(client_hlckmgr), "hlock = %s\n", hlock->lid_.c_str());

    hlock->BeginConverting(true);
    if (!hlock->lock_)
    {
        for (itr = hlock->children_.begin(); itr != hlock->children_.end();
             hlock->children_.erase(itr++))
        {
    ...
    else
    {
                ...
                old_public_mode == lock_protocol::Mode::SR)
            {
                ...
                else
                {
                    ...
                    for (itr = hlock->children_.begin(); itr != hlock->children_.end();
                         hlock->children_.erase(itr++))
                    {
                        assert(DowngradePublicLockRecursive(*itr, lock_protocol::Mode::NL,
                                                            release_set) == 0);
                    }
                }
            }
        ...
        else
        {
            ...
            for (itr = hlock->children_.begin(); itr != hlock->children_.end();
                 hlock->children_.erase(itr++))
            {
                assert(DowngradePublicLockRecursive(*itr, lock_protocol::Mode::NL, release_set) ==
                       0);
            }
        }
    }
...
lock_protocol::status HLockManager::DowngradePublicLock(HLock* hlock, lock_protocol::Mode new_mode)
{
    ...
    for (itr = release_set.begin(); itr != release_set.end(); itr++)
    {
        ...
        if (new_mode == lock_protocol::Mode::NL)
        {
            DBG_LOG(DBG_INFO, DBG_MODULE(client_hlckmgr), "[%d] Revoking hierarchical lock %s\n",
                    id(), hl->lid_.c_str());
            if (hcb_)
            {
                hcb_->OnRelease(hl);
...
int HLockManager::Revoke(Lock* lp, lock_protocol::Mode new_mode)
{
    // ISSUE
    ...
    DBG_LOG(DBG_INFO, DBG_MODULE(client_hlckmgr), "[%d] Revoking hierarchical lock %s: %s to %s\n",
            id(), lp->lid_.c_str(), lp->public_mode_.String().c_str(), new_mode.String().c_str());
...
int HLockManager::LockChain(HLock* hlock, LockId lckarray[])
{
    int nlocks = 0;
    HLock* tmp = hlock;
...
void HLockManager::PrintDebugInfo()
{
    LockMap::iterator itr;
    for (itr = locks_.begin(); itr != locks_.end(); ++itr)
    {
        DBG_LOG(DBG_INFO, DBG_MODULE(client_hlckmgr), "LOCK[%s] = %p\n", itr->first.c_str(),
                itr->second);
    }
}
