...
int PersistentRegion::Header::Load(int fd, Header* header)
{
...
int PersistentRegion::Header::Store(int fd, Header& header)
{
 ...
 *
 */
int PersistentRegion::CreateInternal(const char* pathname, size_t nblocks, size_t block_size,
...
int PersistentRegion::Create(const char* pathname, size_t size)
{
 ...
 */
int PersistentRegion::Open(const char* pathname, size_t size, int flags, PersistentRegion** pregion)
{
    int ret;
    int fd = -1;
    int mmap_flags;
    ...
    *pregion = NULL;
    ...
    if (flags & PersistentRegion::kCreate)
    {
        ...
        {
            size_t nblocks = NumOfBlocks(size, kBlockSize);
            if ((ret = PersistentRegion::CreateInternal(pathname, nblocks, kBlockSize, kBlockSize,
                                                        flags)) < 0)
            {
    ...
    base_addr = mmap((void*) mmap_addr, header.length(), PROT_WRITE | PROT_READ, mmap_flags, fd,
                     header.gap());
    ...
    if ((header_mmap_addr = mmap((void*) 0, header.gap(), PROT_WRITE | PROT_READ, MAP_SHARED, fd,
                                 0)) == (void*) -1)
    {
        ret = -E_ERRNO;
        goto done;
...
int PersistentRegion::Open(const char* pathname, PersistentRegion** pregion)
{
    return Open(pathname, 0, 0, pregion);
}

int PersistentRegion::Close(PersistentRegion* pregion)
{
