...
{
  public:
    ExtentDescriptor(osd::common::ExtentId eid, int index) : eid_(eid), index_(index)
    {
    }
...
{
  public:
    ContainerDescriptor(osd::common::ObjectId oid, int index) : oid_(oid), index_(index)
    {
    }
...
class DescriptorPool
{
  public:
    ...
    int AllocateContainer(OsdSession* session, StorageAllocator* salloc, int type,
                          osd::common::ObjectId* oid);
    int AllocateExtent(OsdSession* session, StorageAllocator* salloc, size_t nbytes,
...
{
    typedef google::dense_hash_map<::osd::common::ObjectType,
                                   ::osd::server::ContainerAbstractFactory*>
        ObjectType2Factory;
  ...
  public:
    ...
    int Init();
    /** Shutdowns the allocator, and destroys the object if is was created using one of the factory
     * methods */
    int Close();
    ...
    int CreateObjectIdSet(OsdSession* session, osd::common::AclIdentifier acl_id,
                          ObjectIdSet** obj_set);
    int AllocateObjectIdSet(OsdSession* session, osd::common::AclIdentifier acl_id,
    ...
    int AllocateExtentFromSet(OsdSession* session, osd::common::ObjectId set_oid,
                              osd::common::ExtentId eid, int index_hint);
    int AllocateContainer(OsdSession* session, osd::common::AclIdentifier acl_id, int type,
                          osd::common::ObjectId* oidp);
    int AllocateContainerIntoSet(OsdSession* session, ObjectIdSet* set, int type, int count);
    int AllocateContainerFromSet(OsdSession* session, osd::common::ObjectId set_oid,
                                 osd::common::ObjectId oid, int index_hint);
    int GetDescriptorPool(OsdSession* session, osd::common::AclIdentifier acl_id,
    ...
    int RegisterType(::osd::common::ObjectType type_id,
    ...
    class IpcHandlers
    {
      public:
        ...
        int AllocateObjectIdSet(int clt, osd::common::AclIdentifier acl_id,
        ...
        int AllocateContainerVector(
            int clt, std::vector<::osd::StorageProtocol::ContainerRequest> container_request_vector,
