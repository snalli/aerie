...
{
  public:
    Inode() : oid_(osd::common::ObjectId(0)), ino_(0), type_(0)
    {
    }

    Inode(InodeNumber ino, int type) : oid_(osd::common::ObjectId(ino)), ino_(ino), type_(type)
    {
    }
...
template <typename T> class InodeT : public Inode
{
  public:
    InodeT()
    {
    ...
    InodeT(InodeNumber ino, int type) : Inode(ino, type)
    {
    }
