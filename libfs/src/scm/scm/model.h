 ...
 */
template <typename T> void ScmStore(volatile T* addr, T val)
{
    *addr = val;
    ScmFlush(addr);
}

inline void ScmFence()
{
    asm_mfence();
    emulate_latency_ns(STAMNOS_SCM_LATENCY_WRITE);
}

inline void ScmLatency()
{
    emulate_latency_ns(STAMNOS_SCM_LATENCY_WRITE);
}
 ...
 */
inline void ScmFlush(volatile void* addr)
{
    asm_mfence();
...
inline void* ScmMemCopy(void* dst, const void* src, size_t n)
{
