...
int ArrayContainer<T>::Object<Session>::Read(Session* /*session*/, int index, T* val)
{
    *val = array_[index];
...
template <typename T>
template <typename Session>
int ArrayContainer<T>::Object<Session>::Write(Session* /*session*/, int index, T val)
{
    array_[index] = val;
    return E_SUCCESS;
}

template <typename T> template <typename Session> int ArrayContainer<T>::Object<Session>::Size()
{
    return ARRAY_CONTAINER_SIZE;
}

// Dan Bernstein's hash function
static unsigned long __attribute__((unused)) ArrayHash(const char* str)
{
