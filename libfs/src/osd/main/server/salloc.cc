...
int DescriptorPool::Load(OsdSession* session)
{
...
int DescriptorPool::Create(OsdSession* session, osd::common::ObjectId set_oid,
...
int DescriptorPool::AllocateContainer(OsdSession* session, StorageAllocator* salloc, int type,
                                      osd::common::ObjectId* oid)
{
    int ret;
...
int DescriptorPool::AllocateExtent(OsdSession* session, StorageAllocator* salloc, size_t nbytes,
                                   osd::common::ExtentId* eid)
{
    int ret;
...
StorageAllocator::StorageAllocator(::server::Ipc* ipc, StoragePool* pool, FreeSet* freeset)
    : ipc_(ipc), pool_(pool), freeset_(freeset)
{
...
int StorageAllocator::Load(::server::Ipc* ipc, StoragePool* pool, StorageAllocator** sallocp)
{
    void* b;
    ...
    DBG_LOG(DBG_INFO, DBG_MODULE(server_salloc), "Load storage pool 0x%lx\n", pool->Identity());
...
int StorageAllocator::LoadFreeMap()
{
    ...
    for (int i = 0; i < freeset_->Size(); i++)
    {
        ...
        freemap_.insert(
            std::pair<osd::common::AclIdentifier, ObjectIdSet*>(aclobjpair.acl_id, oid_set));
    }

    return E_SUCCESS;
}

int StorageAllocator::RegisterBaseTypes()
{
    int ret;

    {
        // NameContainer
        ::osd::server::ContainerAbstractFactory* factory =
            new osd::containers::server::NameContainer::Factory();
...

    {
        // ByteContainer
        ::osd::server::ContainerAbstractFactory* factory =
            new osd::containers::server::ByteContainer::Factory();
...

    {
        // NeedleContainer
        ::osd::server::ContainerAbstractFactory* factory =
            new osd::containers::server::NeedleContainer::Factory();
...
int StorageAllocator::RegisterType(::osd::common::ObjectType type_id,
...
int StorageAllocator::GetDescriptorPool(OsdSession* session, osd::common::AclIdentifier acl_id,
                                        DescriptorPool** poolp)
{
    int ret;
    ...
    DBG_LOG(DBG_INFO, DBG_MODULE(server_salloc), "Allocate container set of ACL %d\n", acl_id);
...
// OBSOLETE
int StorageAllocator::Alloc(size_t /*nbytes*/, std::type_info const& /*typid*/, void** /*ptr*/)
{
    assert(0);
    return 0;
}
...
// OBSOLETE
int StorageAllocator::Alloc(OsdSession* /*session*/, size_t /*nbytes*/,
                            std::type_info const& /*typid*/, void** /*ptr*/)
{
    assert(0);
    return 0;
}
...
int StorageAllocator::AllocateExtent(OsdSession* /*session*/, size_t size, int /*flags*/,
                                     void** ptr)
{
    int ret;

    DBG_LOG(DBG_INFO, DBG_MODULE(server_salloc), "Allocating Extent: size=%lu (0x%lx)...\n", size,
            size);
...
int StorageAllocator::FreeExtent(OsdSession* /*session*/, osd::common::ExtentId eid)
{
    int ret;

    DBG_LOG(DBG_INFO, DBG_MODULE(server_salloc), "Freeing Extent: %p ...\n", eid.u64());
...
int StorageAllocator::AllocateExtentIntoSet(OsdSession* session, ObjectIdSet* set, int size,
                                            int count)
{
    PROFILER_PREAMBLE
    int ret;
    ...
    DBG_LOG(DBG_INFO, DBG_MODULE(server_salloc), "[%d] Allocate %d extents(s) of size %d\n",
...
// not thread safe
int StorageAllocator::CreateObjectIdSet(OsdSession* session, osd::common::AclIdentifier /*acl_id*/,
                                        ObjectIdSet** obj_set)
{
    char* buffer;
    size_t extent_size;
...
int StorageAllocator::AllocateObjectIdSet(OsdSession* session, osd::common::AclIdentifier acl_id,
                                          osd::common::ObjectId* set_oid)
{
    int ret;
    FreeMap::iterator it;
    ...
    DBG_LOG(DBG_INFO, DBG_MODULE(server_salloc), "[%d] Allocate ObjectID Set of ACL %d\n",
            session->clt(), acl_id);
    ...
    {
        oid = it->second->oid();
        if ((obj_set = osd::containers::server::SetContainer<osd::common::ObjectId>::Object::Load(
                 oid)) == NULL)
        {
            ret = -E_NOMEM;
            goto done;
...
int StorageAllocator::AllocateContainer(OsdSession* session, osd::common::AclIdentifier acl_id,
...
int StorageAllocator::FreeContainer(OsdSession* /*session*/, osd::common::ObjectId oid)
{
    int type = oid.type();
    assert(type < 16);
...
int StorageAllocator::AllocateContainerIntoSet(OsdSession* session, ObjectIdSet* set, int type,
                                               int count)
{
    int ret;
    char* buffer;
    ...
    DBG_LOG(DBG_INFO, DBG_MODULE(server_salloc), "[%d] Allocate %d container(s) of type %d\n",
...
int StorageAllocator::AllocateContainerFromSet(OsdSession* session, osd::common::ObjectId set_oid,
                                               osd::common::ObjectId /*oid*/, int index_hint)
{
    ObjectIdSet* obj_set;

    DBG_LOG(DBG_INFO, DBG_MODULE(server_salloc),
            "[%d] Allocate container %p from set %p (hint=%d)\n", session->clt(), (void*) oid.u64(),
            (void*) set_oid.u64(), index_hint);
...
int StorageAllocator::AllocateExtentFromSet(OsdSession* session, osd::common::ObjectId set_oid,
                                            osd::common::ExtentId /*eid*/, int index_hint)
{
    ObjectIdSet* obj_set;

    DBG_LOG(DBG_INFO, DBG_MODULE(server_salloc), "[%d] Allocate extent %p from set %p (hint=%d)\n",
            session->clt(), (void*) eid.u64(), (void*) set_oid.u64(), index_hint);
...
int StorageAllocator::IpcHandlers::Register(StorageAllocator* salloc)
{
    salloc_ = salloc;
...
int StorageAllocator::IpcHandlers::AllocateObjectIdSet(int clt, osd::common::AclIdentifier acl_id,
                                                       ::osd::StorageProtocol::ContainerReply& r)
{
    ...
    if ((ret = salloc_->AllocateObjectIdSet(static_cast<OsdSession*>(session), acl_id, r)) < 0)
    {
        return -ret;
    }
    return E_SUCCESS;
...
int StorageAllocator::IpcHandlers::AllocateContainerIntoSet(int clt, int set_capability, int type,
                                                            int num, int& /*reply*/)
{
    int ret;
    ::server::BaseSession* session;
...
// set_capability is an index into the table of container sets allocates to the client
// new containers will be allocated into the set pointed by the set_capability
int StorageAllocator::IpcHandlers::AllocateExtentIntoSet(int clt, int set_capability, int size,
                                                         int num, int& /*reply*/)
{
    int ret;
    ::server::BaseSession* session;
...
int StorageAllocator::IpcHandlers::AllocateContainerVector(
    int /*clt*/, std::vector<::osd::StorageProtocol::ContainerRequest> container_req_vec,
