...
int File::Init(Inode* ip, int flags)
{
...
int File::Write(client::Session* session, const char* src, uint64_t n)
{
...
int File::Read(client::Session* session, char* dst, uint64_t n)
{
    int ret;
...
int File::Write(client::Session* session, const char* src, uint64_t n, uint64_t offset)
{
    int ret;
...
int File::Read(client::Session* session, char* dst, uint64_t n, uint64_t offset)
{
    int ret;
...
uint64_t File::Seek(client::Session* session, uint64_t offset, int whence)
{
    uint64_t size;
    switch (whence)
    {
...
int FileManager::AllocFd(File* fp)
{
    ...
    pthread_mutex_lock(&mutex_);
    if ((fd = AllocFd(0)) < 0)
    { // insight : I don't understand this.
        pthread_mutex_unlock(&mutex_);
        return -1;
    }
