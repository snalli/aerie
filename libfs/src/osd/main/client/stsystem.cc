...
int StorageSystem::Open(const char* source, unsigned int /*flags*/)
{
    int ret;
...
int StorageSystem::Mount(const char* source, const char* /*target*/, unsigned int flags,
...
int StorageSystem::Mount(const char* source, unsigned int flags, StorageSystemDescriptor& desc)
{
...
int StorageSystem::Mount(const char* source, const char* target, unsigned int flags)
{
    ...
    if ((ret = ipc_->call(StorageSystemProtocol::kMount, ipc_->id(), std::string(source), flags,
                          mntrep)) < 0)
    {
        return -E_IPC;
    }
...
int StorageSystem::Mount(const char* source, unsigned int flags)
{
    return Mount(source, NULL, flags);
}

int StorageSystem::Close()
{
    return 0;
}
