...
int Interval::WriteBlock(OsdSession* session, char* src, uint64_t bn, int off, int n)
{
    int ret;
    char* bp;
    void* ptr = NULL;
...
int Interval::Write(OsdSession* session, char* src, uint64_t off, uint64_t n)
{
    return osd::containers::common::__Write<OsdSession, Interval>(session, this, src, off, n);
}

int Interval::ReadBlock(OsdSession* session, char* dst, uint64_t bn, int off, int n)
{
    char* bp;
...
int Interval::Read(OsdSession* session, char* dst, uint64_t off, uint64_t n)
{
...
int ByteContainer::VersionManager::ReadShadow(OsdSession* session, char* dst, uint64_t off,
                                              uint64_t n)
{
    int ret = 0;
    int w;
...
{
    if (osd::containers::client::ByteContainer::Object* object =
            (osd::containers::client::ByteContainer::Object*) 0x80fffc0700)
    {
...
void print_debug(OsdSession* session, osd::containers::client::ByteContainer::Object* object,
                 bool incr_counter = true)
{
    if (incr_counter)
        __counter++;
    if (__debug)
    {
        printf("[%llu] Object: %p\n", __counter, object);
        osd::containers::client::ByteContainer::Object* tmp_object =
            (osd::containers::client::ByteContainer::Object*) 0x80fffc0700;
...
void ByteContainer::VersionManager::PrintIntervals()
{
    IntervalTree::iterator interval_it;

    for (interval_it = intervaltree_->begin(); interval_it != intervaltree_->end(); interval_it++)
    {
