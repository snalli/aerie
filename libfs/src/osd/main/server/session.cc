...
int OsdSession::Init(int clt)
{
    ...
    if ((shbuf_ = reinterpret_cast<OsdSharedBuffer*>(ipc_->shbuf_manager()->CreateSharedBuffer(
             "OsdSharedBuffer", static_cast<BcsSession*>(this)))) == NULL)
    {
        return -E_NOMEM;
    }
    sprintf(buf, "%d", clt);
...
osd::server::StorageAllocator* OsdSession::salloc()
{
