...
#include "osd/main/common/lock_protocol-static.h"
...
ThreadRecord::ThreadRecord() : tid_(-1), mode_(lock_protocol::Mode(lock_protocol::Mode::NL))
{
}

ThreadRecord::ThreadRecord(id_t tid, Mode mode) : tid_(tid), mode_(mode)
{
}

Lock::Lock(LockId lid = 0)
    : lid_(lid), gtque_(lock_protocol::Mode::CARDINALITY, lock_protocol::Mode::NL), seq_(0),
      used_(false), can_retry_(false), revoke_type_(0),
      public_mode_(lock_protocol::Mode(lock_protocol::Mode::NL)), cancel_(false), payload_(0),
      status_(NONE)
{
...
LockManager::LockManager(::client::Ipc* ipc) : ipc_(ipc), last_seq_(0)
{
    assert(Init() == 0);
}

int LockManager::Init()
{
...
{
    DBG_LOG(DBG_INFO, DBG_MODULE(client_lckmgr), "[%d] Shutting down Lock Manager\n", id());
    ...
    DBG_LOG(DBG_INFO, DBG_MODULE(client_lckmgr), "[%d] Shutting down Lock Manager: DONE\n", id());
}

void LockManager::RegisterLockCallback(LockType type, LockCallback* lcb)
{
    assert(type < LOCK_TYPE_COUNT);
    lcb_[type] = lcb;
}

void LockManager::RegisterLockRevoke(LockType type, LockRevoke* lrvk)
{
    assert(type < LOCK_TYPE_COUNT);
    lrvk_[type] = lrvk;
}

void LockManager::UnregisterLockCallback(LockType type)
{
    assert(type < LOCK_TYPE_COUNT);
    lcb_[type] = NULL;
}

void LockManager::UnregisterLockRevoke(LockType type)
{
    assert(type < LOCK_TYPE_COUNT);
    lrvk_[type] = NULL;
}
...
inline Lock* LockManager::FindLockInternal(LockId lid)
{
    Lock* lp;
...
/// the lock.
/// Assumes caller has the mutex LockManager::mutex_
inline Lock* LockManager::FindOrCreateLockInternal(LockId lid)
{
    Lock* lp;
    ...
    if (lp == NULL)
    {
        DBG_LOG(DBG_INFO, DBG_MODULE(client_lckmgr), "[%d] Creating lock %s\n", id(), lid.c_str());
        lp = new Lock(lid);
        locks_[lid] = lp;
    }
    return lp;
...
Lock* LockManager::FindLock(LockId lid)
{
    Lock* l;
...
///
/// Does no reference counting.
Lock* LockManager::FindOrCreateLock(LockId lid)
{
    Lock* l;
...
void LockManager::Releaser()
{
    ...
    while (releaser_thread_running_)
    {
        ...
        DBG_LOG(DBG_INFO, DBG_MODULE(client_lckmgr),
                ...
                lid.c_str(), l->public_mode_.String().c_str(), seq, l->revoke_type_);
        if (l->status() == Lock::NONE)
        {
            DBG_LOG(DBG_WARNING, DBG_MODULE(client_lckmgr), "[%d] false revoke alarm: %s\n", id(),
                    lid.c_str());
        ...
        if ((lrvk = lrvk_[lid.type()]))
        {
            ...
            lrvk->Revoke(
                l, static_cast<lock_protocol::Mode::Enum>(revoke2mode_table[l->revoke_type_]));
        }
        else
        {
            lock_protocol::Mode new_mode = lock_protocol::Mode(
                static_cast<lock_protocol::Mode::Enum>(revoke2mode_table[l->revoke_type_]));
            // wait until the lock is released or becomes compatible with the
            // revoke-request's mode
            while (
                !((l->status() == Lock::NONE) || (l->status() == Lock::FREE) ||
                  (l->status() == Lock::LOCKED && l->revoke_type_ != lock_protocol::RVK_NL &&
            ...
            if (revoke_map_.find(lid) != revoke_map_.end())
            {
                ...
                {
                    DBG_LOG(DBG_CRITICAL, DBG_MODULE(client_lckmgr), "[%d] why woken up?\n", id());
                }
            }
            pthread_mutex_unlock(&mutex_);
        }
...
void LockManager::ShutdownReleaser()
{
    DBG_LOG(DBG_INFO, DBG_MODULE(client_lckmgr), "[%d] Shutting down Releaser thread\n", id());
...
lock_protocol::status LockManager::AcquireInternal(unsigned long tid, Lock* l,
                                                   ...
                                                   lock_protocol::Mode& mode_granted)
{
    ...
    DBG_LOG(DBG_INFO, DBG_MODULE(client_lckmgr), "[%d] Acquiring lock %s (%s), flags=%d\n", id(),
            lid.c_str(), mode_set.String().c_str(), flags);
...
check_state:
    ...
    case Lock::FREE:
        ...
        if (mode == lock_protocol::Mode(lock_protocol::Mode::NL))
        {
            ...
            if ((r = do_convert(l, mode, flags | Lock::FLG_NOBLK)) == lock_protocol::OK)
            {
                l->public_mode_ = mode;
            }
            else
        ...
        DBG_LOG(DBG_INFO, DBG_MODULE(client_lckmgr), "[%d:%lu] got lock %s at seq %d\n", id(), tid,
                lid.c_str(), l->seq_);
        mode_granted = mode;
    ...
    case Lock::NONE:
               ...
               lock_protocol::RETRY)
        {
            ...
            if (l->cancel_)
            {
                ...
                DBG_LOG(DBG_INFO, DBG_MODULE(client_lckmgr),
                        ...
                        lid.c_str(), mode_granted.String().c_str(), l->seq_);
                l->set_status(Lock::NONE);
        ...
        {
            DBG_LOG(DBG_INFO, DBG_MODULE(client_lckmgr), "[%d:%lu] got lock %s (%s) at seq %d\n",
...
lock_protocol::status LockManager::Acquire(Lock* lock, lock_protocol::Mode::Set mode_set, int flags,
                                           int argc, void** argv, lock_protocol::Mode& mode_granted)
{
...
lock_protocol::status LockManager::Acquire(LockId lid, lock_protocol::Mode::Set mode_set, int flags,
                                           int argc, void** argv, lock_protocol::Mode& mode_granted)
{
    Lock* lock;
    lock_protocol::status r;
...
lock_protocol::status LockManager::ConvertInternal(unsigned long tid, Lock* l,
                                                   lock_protocol::Mode new_mode, bool synchronous)
{
    lock_protocol::status r = lock_protocol::OK;
    LockId lid = l->lid_;
    ...
    {
        DBG_LOG(DBG_INFO, DBG_MODULE(client_lckmgr), "[%d] cannot convert non-locked lock %s\n",
                id(), lid.c_str());
    ...
    {
        DBG_LOG(DBG_INFO, DBG_MODULE(client_lckmgr), "[%d] thread %lu is not holder of lock %s\n",
                id(), tid, lid.c_str());
        r = lock_protocol::NOENT;
    }
    return r;
...
lock_protocol::status LockManager::Convert(Lock* lock, lock_protocol::Mode new_mode,
                                           bool synchronous)
{
    lock_protocol::status r;
...
lock_protocol::status LockManager::Convert(LockId lid, lock_protocol::Mode new_mode,
                                           bool synchronous)
{
    lock_protocol::status r;
    Lock* lock;
...
lock_protocol::status LockManager::ReleaseInternal(unsigned long tid, Lock* l, bool synchronous)
{
    lock_protocol::status r = lock_protocol::OK;

    if (!l)
    {
        DBG_LOG(DBG_INFO, DBG_MODULE(client_lckmgr), "[%d] Releasing unknown lock\n", id());
        return lock_protocol::NOENT;
    }
    else
    {
        DBG_LOG(DBG_INFO, DBG_MODULE(client_lckmgr), "[%d] Releasing lock %s \n", id(),
                l->lid_.c_str());
    }
    ...
    {
        DBG_LOG(DBG_INFO, DBG_MODULE(client_lckmgr), "[%d] calling release RPC for lock %s\n", id(),
                l->lid_.c_str());
        if (do_release(l, 0) == lock_protocol::OK)
        {
            // we set the lock's status to none instead of erasing it
...
lock_protocol::status LockManager::Release(LockId lid, bool synchronous)
{
    lock_protocol::status r;
    Lock* lock;
...
{
    DBG_LOG(DBG_INFO, DBG_MODULE(client_lckmgr), "[%d] Cancel lock %s\n", id(), l->lid_.c_str());
...
lock_protocol::status LockManager::Cancel(LockId lid)
{
    Lock* lock;
    lock_protocol::status r;
...
rlock_protocol::status LockManager::revoke(lock_protocol::LockId lid_u64, int seq, int revoke_type,
                                           int& /*unused*/)
{
 ...
 * was interested.
 */
rlock_protocol::status LockManager::retry(lock_protocol::LockId lid_u64, int seq, int& accepted)
{
    rlock_protocol::status r = rlock_protocol::OK;
    Lock* l;
    LockId lid = LockId(lid_u64);
    ...
    if (seq >= l->seq_)
    {
        ...
        if (l->status() == Lock::NONE)
        {
            ...
            DBG_LOG(DBG_WARNING, DBG_MODULE(client_lckmgr),
                    ...
                    id(), seq, lid.c_str(), l->seq_);
            accepted = false;
        }
        else
        {
            assert(l->status() == Lock::ACQUIRING);
            DBG_LOG(DBG_INFO, DBG_MODULE(client_lckmgr), "[%d] retry message for lid %s seq %d\n",
                    id(), lid.c_str(), seq);
            l->can_retry_ = true;
            accepted = true;
            pthread_cond_signal(&l->retry_cv_);
...
int LockManager::do_acquire(Lock* l, lock_protocol::Mode::Set mode_set, int flags, int argc,
                            void** argv, lock_protocol::Mode& mode_granted)
{
    int r;
    int retval;
    int rpc_flags = 0;
    ...
    r = ipc_->call(lock_protocol::acquire, id(), ++last_seq_, l->lid_.marshall(), mode_set.value(),
...
int LockManager::do_convert(Lock* l, lock_protocol::Mode mode, int flags)
{
    ...
    rpc_flags = flags & (lock_protocol::FLG_NOQUE | lock_protocol::FLG_CAPABILITY);
    r = ipc_->call(lock_protocol::convert, id(), l->seq_, l->lid_.marshall(), mode.value(),
                   rpc_flags, unused);
    return r;
}
...
int LockManager::do_release(Lock* l, int flags)
{
    ...
    r = ipc_->call(lock_protocol::release, id(), l->seq_, l->lid_.marshall(), flags, unused);
    return r;
}

int LockManager::stat(LockId lid)
{
    int r = 0;
