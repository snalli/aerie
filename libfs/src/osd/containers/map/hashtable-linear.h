...
template <typename Session> class Entry
{
  public:
    ...
    static inline Entry* Split(Session* session, Entry* entry, int offset)
    {
        ...
        if (!entry->IsFree() || offset > entry->get_payload_size() - TAG_SIZE + 1)
        {
            return NULL;
        }
        b = entry->tag_;
...
template <typename Session> class Page
{
  public:
    Page()
    {
    ...
    bool IsEmpty()
    {
        ...
        return (entry->get_size() == PAGE_SIZE - sizeof(next_) ? true : false);
    }
  ...
  private:
    ...
    int Delete(Session* session, Entry<Session>* entry, Entry<Session>* prev_entry,
...
int Page<Session>::Insert(Session* session, const char* key, int key_size, const char* val,
                          int val_size)
{
    Entry<Session>* entry;
...
// If no space available then -E_NOMEM is returned
template <typename Session>
int Page<Session>::Insert(Session* session, const char* key, int key_size, uint64_t val)
{
    return Insert(session, key, key_size, (char*) &val, sizeof(val));
}

template <typename Session>
int Page<Session>::Search(Session* /*session*/, const char* key, int key_size, char** valp,
                          int* val_sizep)
{
    unsigned int i;
    int step;
    ...
    for (i = 0; i < PAGE_SIZE - sizeof(next_); i += step)
    {
        entry = GetEntry(i);
        step = entry->get_size();
        if (!entry->IsFree() && (entry->get_keysize() == key_size))
        {
...
template <typename Session>
int Page<Session>::Search(Session* session, const char* key, int key_size, uint64_t* val)
{
    int val_size;
...
template <typename Session>
int Page<Session>::Delete(Session* session, Entry<Session>* entry, Entry<Session>* prev_entry,
                          Entry<Session>* next_entry)
{
    int size;
    ...
    size = entry->get_size();
    size += (next_entry && next_entry->IsFree() == true ? next_entry->get_size() : 0);
...
template <typename Session>
int Page<Session>::Delete(Session* session, const char* key, int key_size)
{
    unsigned int i;
    int step;
    Entry<Session>* entry;
    ...
    for (i = 0; i < PAGE_SIZE - sizeof(next_); i += step)
    {
        entry = GetEntry(i);
        step = entry->get_size();
        if (!entry->IsFree() && (entry->get_payload_size() == key_size + VAL_SIZE))
        {
...
template <typename Session> int Page<Session>::SplitHalf(Session* session, Page* splitover_page)
{
    int ret;
    int i;
    int size;
    ...
    for (i = 0, count = 0; i < PAGE_SIZE - sizeof(next_); i += size)
    {
        ...
        if (!entry->IsFree())
        {
            ...
            if (count > split_count)
            {
                ...
                if ((ret = splitover_page->Insert(session, entry->get_key(), entry->get_keysize(),
                                                  val)) != 0)
                {
                    return ret;
                }
...
// current bucket page and error -E_NOMEM is returned
template <typename Session>
int Page<Session>::Split(Session* session, Page* splitover_page,
...
template <typename Session> int Page<Session>::Merge(Session* session, Page* other_page)
{
    int ret;
    int i;
    int size;
    ...
    for (i = 0, count = 0; i < PAGE_SIZE - sizeof(next_); i += size)
    {
        ...
        if (!entry->IsFree())
        {
            ...
            // writes.
            val = *((uint64_t*) entry->get_val());
            if ((ret = Insert(session, entry->get_key(), entry->get_keysize(), val)) != 0)
            {
                return ret;
            }
            // It's safe to delete entries while iterating as tags
...
template <typename Session> void Page<Session>::Print()
{
    int i;
    uint64_t val;
    int size;
...
template <typename Session> int Page<Session>::return_dentry(void* head_addr)
{
...
template <typename Session> class Bucket
{
  public:
    Bucket()
    {
...
template <typename Session>
int Bucket<Session>::Insert(Session* session, const char* key, int key_size, const char* val,
                            int val_size)
{
    int ret;
    Page<Session>* page;
...
template <typename Session>
int Bucket<Session>::Insert(Session* session, const char* key, int key_size, uint64_t val)
{
    return Insert(session, key, key_size, (char*) &val, sizeof(val));
}

template <typename Session>
int Bucket<Session>::Search(Session* session, const char* key, int key_size, char** valp,
                            int* val_sizep)
{
    PROFILER_PREAMBLE
    int ret;
...
template <typename Session>
int Bucket<Session>::Search(Session* session, const char* key, int key_size, uint64_t* val)
{
    int val_size;
    return Search(session, key, key_size, (char**) &val, &val_size);
}
...
template <typename Session>
int Bucket<Session>::Delete(Session* session, const char* key, int key_size)
{
    int ret;
    Page<Session>* page;
...
template <typename Session>
int Bucket<Session>::Split(Session* session, Bucket* splitover_bucket,
                           const SplitPredicate& split_predicate)
{
    int ret;
    Page<Session>* page;
...
template <typename Session> void Bucket<Session>::Print()
{
    Page<Session>* page;
...
template <typename Session> int Bucket<Session>::return_dentry(void* head_addr)
{
...
template <typename Session> class HashTable
{
  public:
    HashTable() : ncount_(0), size_log2_(5), split_idx_(0)
    {
    }
...
template <typename Session> class HashTable<Session>::LinearSplit : public SplitPredicate
{
  public:
    LinearSplit(uint32_t size_log2) : size_log2_(size_log2)
    {
    }
...
template <typename Session> int HashTable<Session>::Init()
{
    split_idx_ = 0;
    size_log2_ = 5;
    ncount_ = 0;
...
template <typename Session>
uint32_t HashTable<Session>::Index(Session* /*session*/, const char* key, int key_size)
{
    uint32_t idx;
    uint32_t fh;
...
template <typename Session>
int HashTable<Session>::Insert(Session* session, const char* key, int key_size, const char* val,
                               int val_size)
{
    uint32_t idx;
...
template <typename Session>
int HashTable<Session>::Insert(Session* session, const char* key, int key_size, uint64_t val)
{
    return Insert(session, key, key_size, (char*) &val, sizeof(val));
}

template <typename Session>
int HashTable<Session>::Search(Session* session, const char* key, int key_size, char** valp,
                               int* val_sizep)
{
    PROFILER_PREAMBLE
    uint32_t idx;
...
template <typename Session>
int HashTable<Session>::Search(Session* session, const char* key, int key_size, uint64_t* val)
{
    int val_size;
...
template <typename Session>
int HashTable<Session>::Delete(Session* session, const char* key, int key_size)
{
...
template <typename Session> void HashTable<Session>::Print()
{
    int i;
    uint32_t idx;
    Bucket<Session>* bucket;
...
template <typename Session> int HashTable<Session>::return_dentry(void* head_addr)
{
    int i;
    Bucket<Session>* bucket;
