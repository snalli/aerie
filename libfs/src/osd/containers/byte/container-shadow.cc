...
{
  public:
    ByteInterval(const int low, const int high) : low_(low), high_(high)
    {
    }

    ByteInterval(OsdSession* session, ByteContainer::Object* obj, ByteContainer::Slot& slot,
                 const int low, const int size)
        : object_(obj), low_(low), high_(low + size - 1), slot_(slot)
    {
    ...
    inline int GetLowPoint() const
    {
        return low_;
    }
    inline int GetHighPoint() const
    {
        return high_;
    }
...
int ByteInterval::WriteBlockNoRegion(OsdSession* session, char* src, uint64_t bn, int off, int n)
{
    int ret;
    char* bp;
    void* ptr;
    ...
    if (!(bp = block_array_[bn - low_]))
    {
        ...
        session->journal() << osd::Publisher::Message::ContainerOperation::LinkBlock(object_->oid(),
                                                                                     bn, ptr);
...
int ByteInterval::WriteNoRegion(OsdSession* session, char* src, uint64_t off, uint64_t n)
{
    uint64_t tot;
    uint64_t m;
    uint64_t bn;
...
int ByteInterval::Write(OsdSession* session, char* src, uint64_t off, uint64_t n)
{
    if (region_)
    {
...
int ByteInterval::ReadBlockNoRegion(OsdSession* /*session*/, char* dst, uint64_t bn, int off, int n)
{
    char* bp;
...
int ByteInterval::ReadNoRegion(OsdSession* session, char* dst, uint64_t off, uint64_t n)
{
    uint64_t tot;
    uint64_t m;
    uint64_t bn;
...
int ByteInterval::Read(OsdSession* session, char* dst, uint64_t off, uint64_t n)
{
    if (region_)
    {
...
int ByteContainer::VersionManager::vOpen()
{
...
int ByteContainer::VersionManager::vUpdate(OsdSession* session)
{
...
int ByteContainer::VersionManager::ReadImmutable(OsdSession* session, char* dst, uint64_t off,
                                                 uint64_t n)
{
    uint64_t tot;
    uint64_t m;
    ...
    dbg_log(DBG_DEBUG,
            "Immutable range = [%" PRIu64 ", %" PRIu64 ") n=%" PRIu64 ", size=%" PRIu64 "\n", off,
            off + n, n, size_);
    ...
    {
        if (off > size_)
        { // insight : size_ is set by ByteContainer::Object() in osd/containers/name/common.h,
          // initially set to 0
            return 0;
        }
        else
        ...
        dbg_log(DBG_DEBUG,
                "Immutable range pruned = [%" PRIu64 ", %" PRIu64 ") n=%" PRIu64 ", size=%" PRIu64
                "\n",
                off, off + n, n, size_);
    }
    ...
    for (tot = 0, bn = fbn;
         ...
         iter++, tot += m, off += m, bn++) // insight : m is the number of bytes read at a time. I
                                           // want to see how this maps to the SCM ???
    {
        bcount = 1 << (((*iter).slot_height_ - 1) * RADIX_TREE_MAP_SHIFT);
        size = bcount * kBlockSize;
        m = min(n - tot,
                size -
                    f); // insight : Either you read 'size' bytes or whatever is left of 'n' bytes.
                        // insight : "tot" is the number of bytes you read in each iteration
                        //

        ptr = (char*) (*iter).slot_base_[(*iter).slot_offset_];

        // " , %" PRIu64 " ] size=%" PRIu64 "  (%" PRIu64 "  blocks)\n",
...
int ByteContainer::VersionManager::ReadMutable(OsdSession* session, char* dst, uint64_t off,
                                               uint64_t n)
{
    int vn;
...
int ByteContainer::VersionManager::Read(OsdSession* session, char* dst, uint64_t off, uint64_t n)
{
...
int ByteContainer::VersionManager::WriteMutable(OsdSession* session, char* src, uint64_t off,
                                                uint64_t n)
{
    uint64_t bn;
...
int ByteContainer::VersionManager::WriteImmutable(OsdSession* session, char* src, uint64_t off,
                                                  uint64_t n)
{
    s_log("[%ld] ByteContainer::VersionManager::%s", s_tid, __func__);
    uint64_t tot;
            ...
            if (!interval)
            {
                ...
                else
                {
                    ...
                    interval_low = ::osd::containers::common::ByteContainer::N_DIRECT +
                                   ((base_bn - ::osd::containers::common::ByteContainer::N_DIRECT) &
                                    ~(interval_size - 1));
                }
                interval =
                    new ByteInterval(session, object(), (*iter), interval_low, interval_size);
...
int ByteContainer::VersionManager::Write(OsdSession* session, char* src, uint64_t off, uint64_t n)
{
    uint64_t immmaxsize; // immutable range max size
    uint64_t mn;
    int ret1 = 0;
    ...
    dbg_log(DBG_DEBUG,
            "Write range = [%" PRIu64 " , %" PRIu64 " ] n=%" PRIu64 ", immmaxsize=%" PRIu64 "\n",
    ...
    // insight : case 1 (---) |
    {
        s_log("[%ld] ByteContainer::VersionManager::%s case 1 (off + n) = %ld immmaxsize = %ld",
              s_tid, __func__, off + n, immmaxsize);
        ret1 = WriteImmutable(session, src, off, n);
    }
    else if (off >= immmaxsize)
    {
        // insight : case 2 | (---)
        s_log("[%ld] ByteContainer::VersionManager::%s case 2 (off + n) = %ld immmaxsize = %ld",
              s_tid, __func__, off + n, immmaxsize);
    ...
    {
        // insight : case 3 (--|-)
        s_log("[%ld] ByteContainer::VersionManager::%s case 3 (off + n) = %ld immmaxsize = %ld",
              s_tid, __func__, off + n, immmaxsize);
        mn = off + n - immmaxsize;
...
int ByteContainer::VersionManager::Size(OsdSession* /*session*/)
{
    return size_;
}

int ByteContainer::VersionManager::Size()
{
    return size_;
}
