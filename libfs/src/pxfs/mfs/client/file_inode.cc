...
int FileInode::Read(::client::Session* session, char* dst, uint64_t off, uint64_t n)
{
    return rw_ref()->proxy()->interface()->Read(session, dst, off, n);
}

int FileInode::Write(::client::Session* session, char* src, uint64_t off, uint64_t n)
{
    int ret;
...
int FileInode::Lock(::client::Session* session, lock_protocol::Mode mode)
{
...
int FileInode::Lock(::client::Session* session, Inode* parent_inode, lock_protocol::Mode mode)
{
#ifdef FAST_SPIDER_LOCK_PATH
    pthread_mutex_lock(&spider_lock);
#else
    ...
    if (ref_)
    {
        ...
        if (parent_inode->ref_)
        {
            ...
            return cc_proxy->Lock(session, cc_proxy_parent, mode,
                                  osd::cc::client::HLock::FLG_STICKY);
...
int FileInode::Unlock(::client::Session* session)
{
#ifdef FAST_SPIDER_LOCK_PATH
...
int FileInode::Sync(::client::Session* session)
{
    return rw_ref()->proxy()->interface()->vUpdate(session);
}

int FileInode::ioctl(::client::Session* session, int request, void* info)
{
    int ret = E_SUCCESS;
    switch (request)
    {
