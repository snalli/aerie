...
typedef osd::cc::common::LockId LockId;
...
static int revoke_table[8][8] = {
     ...
     */
    /* NL   */ {Lock::RVK_NO, Lock::RVK_NO, Lock::RVK_NO, Lock::RVK_NO, Lock::RVK_NO, Lock::RVK_NO,
                Lock::RVK_NO, Lock::RVK_NO},
    /* SL   */
    {Lock::RVK_NO, Lock::RVK_NO, Lock::RVK_NO, Lock::RVK_NO, Lock::RVK_NO, Lock::RVK_NL,
     Lock::RVK_NL, Lock::RVK_NO},
    /* SR   */
    {Lock::RVK_NO, Lock::RVK_NO, Lock::RVK_NO, Lock::RVK_NO, Lock::RVK_SR2SL, Lock::RVK_NL,
     Lock::RVK_NL, Lock::RVK_SR2SL},
    /* IS   */
    {Lock::RVK_NO, Lock::RVK_NO, Lock::RVK_NO, Lock::RVK_NO, Lock::RVK_NO, Lock::RVK_NO,
     Lock::RVK_NL, Lock::RVK_NO},
    /* IX   */
    {Lock::RVK_NO, Lock::RVK_NO, Lock::RVK_NL, Lock::RVK_NO, Lock::RVK_NO, Lock::RVK_NO,
     Lock::RVK_NL, Lock::RVK_NO},
    /* XL   */
    {Lock::RVK_NO, Lock::RVK_XL2SL, Lock::RVK_NL, Lock::RVK_NO, Lock::RVK_NO, Lock::RVK_NL,
     Lock::RVK_NL, Lock::RVK_XL2SL},
    /* XR   */
    {Lock::RVK_NO, Lock::RVK_XR2IXSL, Lock::RVK_NL, Lock::RVK_XR2XL, Lock::RVK_XR2XL,
     Lock::RVK_XR2IX, Lock::RVK_NL, Lock::RVK_XR2IXSL},
    /* IXSL */
    {Lock::RVK_NO, Lock::RVK_NO, Lock::RVK_NL, Lock::RVK_NO, Lock::RVK_NO, Lock::RVK_IXSL2IX,
     Lock::RVK_NL, Lock::RVK_NO},
};

ClientRecord::ClientRecord() : seq_(-1), clt_(-1), mode_(lock_protocol::Mode::NL)
{
}

ClientRecord::ClientRecord(ClientRecord::id_t clt, int seq, ClientRecord::Mode mode)
    : seq_(seq), clt_(clt), mode_(mode)
{
}

Lock::Lock(lock_protocol::LockId lid)
    : gtque_(lock_protocol::Mode::CARDINALITY, lock_protocol::Mode(lock_protocol::Mode::NL)),
      expected_clt_(-1), retry_responded_(false), revoke_sent_(false), lid_(lid)
{
...
int LockManager::Init(bool register_handler)
{
    pthread_t th __attribute__((unused));

    DBG_LOG(DBG_INFO, DBG_MODULE(server_lckmgr), "Initializing Base Lock Manager\n");
...
Lock* LockManager::FindLockInternal(lock_protocol::LockId lid)
{
    Lock* lp;
...
/// the lock.
/// Assumes caller has the mutex LockManager::mutex_
Lock* LockManager::FindOrCreateLockInternal(lock_protocol::LockId lid)
{
    Lock* lp;

    DBG_LOG(DBG_INFO, DBG_MODULE(client_lckmgr), "Find lock %s\n", LockId(lid).c_str());

    lp = locks_[lid];
    if (lp == NULL)
    {
        DBG_LOG(DBG_INFO, DBG_MODULE(client_lckmgr), "Creating lock %s\n", LockId(lid).c_str());
        lp = new Lock(lid);
        locks_[lid] = lp;
    }
    return lp;
...
Lock* LockManager::FindLock(lock_protocol::LockId lid)
{
    Lock* l;
...
lock_protocol::Mode LockManager::SelectMode(Lock* lock, lock_protocol::Mode::Set mode_set)
{
    ...
    DBG_LOG(DBG_INFO, DBG_MODULE(server_lckmgr), "selecting mode %s out of modes {%s}\n",
            mode.String().c_str(), mode_set.String().c_str());
    return mode;
}

/// assumption: caller has mutex_
lock_protocol::status LockManager::AcquireInternal(int clt, int seq, Lock* l,
                                                   ...
                                                   int& mode_granted)
{
    ...
    DBG_LOG(DBG_INFO, DBG_MODULE(server_lckmgr), "clt %d seq %d acquiring lock %s (%s)\n", clt, seq,
            LockId(l->lid_).c_str(), mode_set.String().c_str());

    mode = SelectMode(l, mode_set);
    wq_len = l->waiting_list_.size();
    DBG_LOG(DBG_INFO, DBG_MODULE(server_lckmgr), "queue len for lock %s: %d\n",
            LockId(l->lid_).c_str(), wq_len);
    if ((wq_len == 0 || (wq_len > 0 && l->expected_clt_ == clt)) && l->gtque_.CanGrant(mode))
    {
        DBG_LOG(DBG_INFO, DBG_MODULE(server_lckmgr), "lock %s is compatible; granting to clt %d\n",
        ...
        if ((flags & lock_protocol::FLG_NOQUE) == 0)
        {
            if (wq_len > 0)
            {
                ...
                DBG_LOG(DBG_INFO, DBG_MODULE(server_lckmgr),
                        "clt %d not expected for lock %s; queued\n", clt, LockId(l->lid_).c_str());
            }
            else
            {
                DBG_LOG(DBG_INFO, DBG_MODULE(server_lckmgr), "queuing clt %d seq %d for lock %s\n",
...
lock_protocol::status LockManager::Acquire(int clt, int seq, lock_protocol::LockId lid,
                                           int mode_set, int flags, unsigned long long /*arg*/,
                                           int& mode_granted)
{
    lock_protocol::status r;
    Lock* lock;
    ...
    r = AcquireInternal(clt, seq, lock, lock_protocol::Mode::Set(mode_set), flags, mode_granted);
    pthread_mutex_unlock(mutex_);
    return r;
}

// convert does not block to avoid any deadlocks.
lock_protocol::status LockManager::ConvertInternal(int clt, int seq, Lock* l,
                                                   lock_protocol::Mode new_mode, int /*flags*/,
                                                   int& /*unused*/)
{
    ...
    if (locks_.find(l->lid_) != locks_.end() && locks_[l->lid_]->gtque_.Exists(clt))
    {
        ...
        (void) cr;
        DBG_LOG(DBG_INFO, DBG_MODULE(server_lckmgr),
                "clt %d convert lck %s at seq %d (%s --> %s)\n", clt, LockId(l->lid_).c_str(), seq,
                cr->mode().String().c_str(), new_mode.String().c_str());
        assert(cr->seq_ == seq);
        ...
        if (l->revoke_sent_ && !l->gtque_.IsModeSet(new_mode) &&
...
// convert does not block to avoid any deadlocks.
lock_protocol::status LockManager::Convert(int clt, int seq, lock_protocol::LockId lid,
...
lock_protocol::status LockManager::Release(int clt, int seq, lock_protocol::LockId lid, int flags,
                                           int& unused)
{
    lock_protocol::status r;
    Lock* lock;

    DBG_LOG(DBG_INFO, DBG_MODULE(server_lckmgr), "clt %d release lock %s at seq %d\n", clt,
            LockId(lid).c_str(), seq);

    pthread_mutex_lock(mutex_);
    lock = FindOrCreateLockInternal(lid);
    r = ConvertInternal(clt, seq, lock, lock_protocol::Mode(lock_protocol::Mode::NL), flags,
                        unused);
    pthread_mutex_unlock(mutex_);
    return r;
}

lock_protocol::status LockManager::Stat(lock_protocol::LockId /*lid*/, int& r)
{
...
struct RevokeMsg
{
    RevokeMsg(int clt, int seq, int revoke_type) : clt_(clt), seq_(seq), revoke_type_(revoke_type)
    {
    }
...
void LockManager::revoker()
{
    ...
    while (true)
    {
        ...
        // the mutex.
        revoke_msgs.clear();
        for (itr_icr = l->gtque_.begin(); itr_icr != l->gtque_.end(); itr_icr++)
        {
        ...
        for (itr_r = revoke_msgs.begin(); itr_r != revoke_msgs.end(); itr_r++)
        {
            ...
            DBG_LOG(DBG_INFO, DBG_MODULE(server_lckmgr),
                    "revoke client %d lock %s: revoke type = %d\n", clt, LockId(lid).c_str(),
                    (*itr_r).revoke_type_);
            if (cl_dsc)
            {
                if (cl_dsc->call(rlock_protocol::revoke, lid, (*itr_r).seq_, (*itr_r).revoke_type_,
                                 unused) != rlock_protocol::OK)
                {
                    DBG_LOG(DBG_ERROR, DBG_MODULE(server_lckmgr), "failed to send revoke\n");
                }
            }
            else
            {
                DBG_LOG(DBG_ERROR, DBG_MODULE(server_lckmgr), "client %d didn't subscribe\n", clt);
            }
        }
...
void LockManager::retryer()
{
    ...
    while (true)
    {
        ...
        if (found_cr)
        {
            ...
            if (cl_dsc->call(rlock_protocol::retry, lid, cr.seq_, accepted) == rlock_protocol::OK)
            {
                DBG_LOG(DBG_INFO, DBG_MODULE(server_lckmgr),
            ...
            {
                DBG_LOG(DBG_ERROR, DBG_MODULE(server_lckmgr),
