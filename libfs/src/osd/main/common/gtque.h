#ifndef __STAMNOS_OSD_COMMON_GRANT_QUEUE_H
#define __STAMNOS_OSD_COMMON_GRANT_QUEUE_H
...
template <class MemberType> class GrantQueue
{
  public:
    ...
    bool IsModeSet(typename MemberType::Mode mode)
    {
        return (mode_union_.Exists(mode));
    }
    int Size()
    {
...
GrantQueue<MemberType>::GrantQueue(int nmode, typename MemberType::Mode no_member_mode)
    : mode_union_(0), no_member_mode_(no_member_mode)
{
...
template <class MemberType> void GrantQueue<MemberType>::Add(const MemberType& cr)
{
...
template <class MemberType> int GrantQueue<MemberType>::Remove(typename MemberType::id_t id)
{
    if (Exists(id) == false)
    {
        return -1;
...
template <class MemberType> bool GrantQueue<MemberType>::Exists(typename MemberType::id_t id)
{
    return (members_.find(id) != members_.end());
}

template <class MemberType> MemberType* GrantQueue<MemberType>::Find(typename MemberType::id_t id)
{
...
template <class MemberType> int GrantQueue<MemberType>::PartialOrder(typename MemberType::Mode mode)
{
    return MemberType::Mode::Set::PartialOrder(mode, mode_union_);
}

template <class MemberType> typename MemberType::Mode GrantQueue<MemberType>::MostSevere()
{
    return mode_union_.MostSevere(no_member_mode_);
}

template <class MemberType> bool GrantQueue<MemberType>::CanGrant(typename MemberType::Mode mode)
{
...
template <class MemberType> int GrantQueue<MemberType>::Grant(const MemberType& cr)
{
    if (CanGrant(cr.mode()))
    {
        Add(cr);
...
template <class MemberType>
bool GrantQueue<MemberType>::CanConvertInPlace(typename MemberType::id_t id,
...
template <class MemberType>
int GrantQueue<MemberType>::ConvertInPlace(typename MemberType::id_t id,
                                           typename MemberType::Mode new_mode)
{
    if (Exists(id) == false)
    {
...
template <class MemberType> void GrantQueue<MemberType>::Print()
{
    typename std::map<typename MemberType::id_t, MemberType>::iterator itr_icr;

    printf("MEMBERS: %d\n", members_.size());
    for (itr_icr = members_.begin(); itr_icr != members_.end(); itr_icr++)
    {
...
#endif // __STAMNOS_OSD_COMMON_GRANT_QUEUE_H
