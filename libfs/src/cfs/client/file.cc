...
int File::Init(InodeNumber ino, int flags)
{
    if (flags & O_RDWR)
    {
        readable_ = true;
...
// not thread safe
int File::WriteInternal(const char* src, uint64_t n, uint64_t offset)
{
    int ret;
    int reply;
    ...
    if ((ret = global_ipc_layer->call(FileSystemProtocol::kWrite, global_ipc_layer->id(), ino,
                                      (int) n, (int) offset, reply)) < 0)
    {
        return -E_IPC;
    }
    if (ret > 0)
...
int File::ReadInternal(char* dst, uint64_t n, uint64_t offset)
{
    int ret;
    int reply;
    FileSystemProtocol::InodeNumber ino = ino_;
    ...
    if ((ret = global_ipc_layer->call(FileSystemProtocol::kRead, global_ipc_layer->id(), ino,
                                      (int) n, (int) offset, reply)) < 0)
    {
        return -E_IPC;
    }
...
int File::Write(const char* src, uint64_t n)
{
    int ret;
...
int File::Read(char* dst, uint64_t n)
{
    int ret;
...
int File::Write(const char* src, uint64_t n, uint64_t offset)
{
    int ret;
...
int File::Read(char* dst, uint64_t n, uint64_t offset)
{
    int ret;
...
static boost::dynamic_bitset<>::size_type find_first_zero(boost::dynamic_bitset<>& fdset,
                                                          boost::dynamic_bitset<>::size_type start)
{
    ...
    for (bit_set = fdset.find_first(); bit_set != boost::dynamic_bitset<>::npos;
...
int FileManager::AllocFd(int start)
{
    int fd;
    boost::dynamic_bitset<>::size_type first_bit_zero;
...
int FileManager::AllocFd(File* fp)
{
    int fd;
...
int FileManager::AllocFile(File** fpp)
{
    *fpp = new File();
    (*fpp)->ref_ = 1;
    return 0;
...
int FileManager::ReleaseFile(File* fp)
{
    fp->Release();
    delete fp;
    return 0;
...
int FileManager::Lookup(int fd, File** fpp)
{
    File* fp;
...
int FileManager::Get(int fd, File** fpp)
{
    File* fp;
    int newfd;
