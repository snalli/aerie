...
/*
 * Each block scmdisk device has a radix_tree brd_pages of pages that stores
			...
			unsigned int cmd, unsigned long arg)
{
	...
	/*
	 * scm device BLKFLSBUF has special semantics, we want to actually
	 * release and destroy the scmdisk data.
	 */
 ...
 */
static int rd_nr;
int rd_size = CONFIG_BLK_DEV_SCM_SIZE;
static int max_part;
static int part_shift;
module_param(rd_nr, int, 0);
MODULE_PARM_DESC(rd_nr, "Maximum number of brd devices");
module_param(rd_size, int, 0);
MODULE_PARM_DESC(rd_size, "Size of each SCM disk in kbytes.");
module_param(max_part, int, 0);
MODULE_PARM_DESC(max_part, "Maximum number of partitions per SCM disk");
MODULE_LICENSE("GPL");
MODULE_ALIAS_BLOCKDEV_MAJOR(SCMDISK_MAJOR);
MODULE_ALIAS("rd");

#ifndef MODULE
/* Legacy boot options - nonmodular */
static int __init scmdisk_size(char *str)
{
	rd_size = simple_strtol(str, NULL, 0);
	return 1;
}
__setup("scmdisk_size=", scmdisk_size);
#endif
...
static struct brd_device *brd_alloc(int i)
{
	...
	if (!disk)
		goto out_free_queue;
	disk->major		= SCMDISK_MAJOR;
	disk->first_minor	= i << part_shift;
	disk->fops		= &brd_fops;
	disk->private_data	= brd;
	disk->queue		= brd->brd_queue;
	disk->flags |= GENHD_FL_SUPPRESS_PARTITION_INFO;
	sprintf(disk->disk_name, "scm%d", i);
	set_capacity(disk, rd_size * 2);

	/* Initialize SCM layer */
	scm_init(&brd->scm);
...
static int __init brd_init(void)
{
	...
	blk_register_region(MKDEV(SCMDISK_MAJOR, 0), range,
