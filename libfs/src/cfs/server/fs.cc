...
FileSystem::FileSystem(Ipc* ipc, StorageSystem* storage_system)
    : ipc_(ipc), storage_system_(storage_system)
{
...
int FileSystem::Make(const char* target, size_t /*nblocks*/, size_t /*block_size*/, int flags)
{
    int ret;
...
int FileSystem::Load(Ipc* ipc, const char* source, unsigned int flags, FileSystem** fsp)
{
    int ret;
...
int FileSystem::MakeDir(Session* session, const char* path, unsigned int /*mode*/,
                        FileSystemProtocol::InodeNumber& ino)
{
    ...
    if ((ret = cp->Link(session, ".", cp)) < 0)
    {
        return ret;
    }
    if ((ret = cp->Link(session, "..", pp)) < 0)
    {
        return ret;
    }
...
int FileSystem::MakeFile(Session* session, const char* path, unsigned int /*flags*/,
                         unsigned int /*mode*/, FileSystemProtocol::InodeNumber& ino)
{
    int ret;
    char name[128];
    DirInode dinode1;
...
int FileSystem::Read(Session* session, FileSystemProtocol::InodeNumber ino, void* buf, int count,
                     int offset, int& n)
{
    FileInode finode;
...
int FileSystem::Write(Session* session, FileSystemProtocol::InodeNumber ino, void* buf, int count,
                      int offset, int& n)
{
    FileInode finode;
    InodeNumber fino = ino;
...
int FileSystem::Link(Session* session, const char* oldpath, const char* newpath)
{
    int ret;
    char newname[128];
    DirInode dinode1;
...
int FileSystem::Unlink(Session* session, const char* path)
{
    int ret;
    char name[128];
    DirInode dinode1;
...
int FileSystem::Namei(Session* session, const char* path, FileSystemProtocol::InodeNumber& ino)
{
    int ret;
...
int FileSystem::Mount(int clt, const char* source, const char* /*target*/, unsigned int flags,
...
int FileSystem::IpcHandlers::Register(FileSystem* module)
{
    module_ = module;
    ...
    module_->ipc_->reg(::FileSystemProtocol::kRead, this, &::server::FileSystem::IpcHandlers::Read);
    module_->ipc_->reg(::FileSystemProtocol::kLink, this, &::server::FileSystem::IpcHandlers::Link);
...
int FileSystem::IpcHandlers::MakeDir(unsigned int clt, std::string path, unsigned int mode,
                                     FileSystemProtocol::InodeNumber& ino)
{
    int ret;
    ::server::BaseSession* basesession;
...
int FileSystem::IpcHandlers::MakeFile(unsigned int clt, std::string path, unsigned int flags,
                                      unsigned int mode, FileSystemProtocol::InodeNumber& ino)
{
    int ret;
    ::server::BaseSession* basesession;
...
int FileSystem::IpcHandlers::Read(unsigned int clt, FileSystemProtocol::InodeNumber ino, int count,
                                  int offset, int& r)
{
    int ret;
    ::server::BaseSession* basesession;
...
int FileSystem::IpcHandlers::Write(unsigned int clt, FileSystemProtocol::InodeNumber ino, int count,
                                   int offset, int& r)
{
    int ret;
    ::server::BaseSession* basesession;
...
int FileSystem::IpcHandlers::Link(unsigned int clt, std::string oldpath, std::string newpath,
                                  int& /*r*/)
{
    int ret;
    ::server::BaseSession* basesession;
...
int FileSystem::IpcHandlers::Unlink(unsigned int clt, std::string path, int& /*r*/)
{
    int ret;
    ::server::BaseSession* basesession;
    ::server::Session* session;
...
int FileSystem::IpcHandlers::Namei(unsigned int clt, std::string path,
                                   FileSystemProtocol::InodeNumber& ino)
{
    int ret;
    ::server::BaseSession* basesession;
...
int FileSystem::IpcHandlers::Mount(unsigned int clt, std::string source, std::string target,
                                   unsigned int flags, FileSystemProtocol::MountReply& rep)
{
    int ret;

    if ((ret = module_->Mount(clt, source.c_str(), target.c_str(), flags, rep)) < 0)
    {
        return -ret;
    }
    return 0;
