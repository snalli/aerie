...
{
  public:
    Buffer() : size_(128 * 1024), count_(0)
    {
    }
...
int Buffer::Write(const void* src, size_t n)
{
...
int Buffer::Flush(OsdSharedBuffer* dst)
{
    int ret;
...
{
  public:
    Journal(OsdSession* session) : session_(session), mode_(osd::common::Journal::Client)
    {
    }
    ...
    operator<<(Journal* journal, const osd::Publisher::Message::ContainerOperationHeader& header)
    {
        journal->Write(&header, sizeof(header) + header.payload_size_);
        return journal;
    }

    inline friend Journal* operator<<(Journal* journal,
                                      const osd::Publisher::Message::LogicalOperationHeader& header)
    {
        journal->Write(&header, sizeof(header) + header.payload_size_);
        return journal;
    }
