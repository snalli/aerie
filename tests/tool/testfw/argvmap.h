...
inline int extract_kv(std::string& str, std::string& strk, std::string& strv)
{
    size_t start;
    size_t end;
...
inline int ArgValMap::Init(std::string argstr)
{
    std::string substr;
    std::string strk;
    std::string strv;
...
inline int ArgValMap::Merge(ArgValMap& other, bool overwrite)
{
...
inline int ArgValMap::ArgVal(std::string& key, std::string& val)
{
    if (argvmap_.find(key) != argvmap_.end())
    {
        val = argvmap_[key];
...
inline std::string* ArgValMap::ArgVal(std::string key)
{
    if (argvmap_.find(key) != argvmap_.end())
    {
        return &argvmap_[key];
...
inline void ArgValMap::Print(std::ostream& os)
{
    std::map<std::string, std::string>::iterator it;
