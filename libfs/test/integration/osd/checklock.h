...
static int check_grant_x(CountRegion* region, lock_protocol::LockId lid)
{
...
static int check_grant_s(CountRegion* region, lock_protocol::LockId lid)
{
    pthread_mutex_lock(&region->count_mutex);
    int x = lid & 0xff;
    if ((region->ct[x] && lock_mask) == lock_x)
...
static int check_release(CountRegion* region, lock_protocol::LockId lid)
{
    pthread_mutex_lock(&region->count_mutex);
    int x = lid & 0xff;
    if (region->ct[x] == 0)
