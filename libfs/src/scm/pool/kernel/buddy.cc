 ...
 */
static const char* clearCheck(struct PoolInfo* pi, size_t firstBit, size_t pastBit)
{
...
static void makeFree(struct PoolInfo* pi, size_t firstFree, int bitsFree, int zap)
{
    size_t* ptr;
    size_t* pastPtr;
    size_t* nextPtr;
    ...
    if (nextOff != OURNULL)
    {
        size_t* nextPtr = (size_t*) (pi->poolStart + (nextOff << pi->minBits));
...
static void toFree(struct PoolInfo* pi, size_t firstBlock, size_t pastBlock, int zap)
{
    size_t i;
    int tryBits = 0;
    size_t tryLen = 1;
...
const char* poolRelease(struct PoolInfo* pi, size_t startBlock, size_t blocks)
{
    size_t pastBlockOff;
    const char* ret;
    if (startBlock >= pi->poolBlocks || startBlock + blocks > pi->poolBlocks)
        return "bad parms";
...
static const char* poolBuddyMalloc(struct PoolInfo* pi, int bits, void** newStore)
{
    int retBits;
    size_t* ptr;
    size_t* pastPtr;
    ...
    */
    ...
    if (ptr[OFF_PREV] != OURNULL)
        return "corrupted free area";
    pastPtr = (size_t*) (((char*) ptr) + (1UL << (retBits + pi->minBits)));
    ...
    if (memAny(ptr + HEADER_LEN, (1UL << (bits + pi->minBits)) - FREEBLOCK * sizeof(void*)))
        return "corrupted free area";
#endif
    ...
    if (nextOffset != OURNULL)
    {
        ...
        if (nextPtr[OFF_MAGIC] != MAGIC || nextPtr[OFF_PREV] != offset)
            return "corrupted free area";
...
static const char* poolBuddyAllocSize(struct PoolInfo* pi, void* sss, int* numBits)
{
    size_t byteOff;
    size_t blockOff;
    ...
    {
        if ((blockOff + blocks > pi->poolBlocks) || (ourBits >= pi->numSizes) ||
            (blockOff & (blocks - 1)))
        {
...
const char* poolRealloc(struct PoolInfo* pi, void* ptrv, size_t size, void** newPtr)
{
    int canBits;
    size_t blockNo;
    int bits;
...
const char* poolFree(struct PoolInfo* pi, void* storage2)
{
    size_t* sptr;
    size_t* nextPtr;
    size_t nextOffset;
    ...
    { /* while you can merge two blocks and get a legal
     block size */
        ...
        pastPtr =
            (size_t*) (pi->poolStart + (buddy << pi->minBits) + (1UL << (bits + pi->minBits)));
        ...
        else
        {
            ...
            prevPtr = (size_t*) (pi->poolStart + (prevOffset << pi->minBits));
...
const char* poolCheck(struct PoolInfo* pi, size_t* counts)
{
    ...
    for (size = 0; size < pi->numSizes; size++)
    {
        ...
        size_t blocksSeen = 0;
        for (offset = pi->freeList[size]; offset != OURNULL && blocksSeen < maxBlocks; blocksSeen++)
        {
            if (offset >= pi->poolBlocks)
                return "bad pointer";
            if (testAny(pi, offset, offset + (1UL << size)))
            ...
            pastPtr = (size_t*) (((char*) ptr) + (1UL << (pi->minBits + size)));
            ...
            if (memAny(ptr + HEADER_LEN,
                       (1UL << (size + pi->minBits)) - FREEBLOCK * sizeof(size_t)))
            {
