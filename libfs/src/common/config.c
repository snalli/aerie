...
int
my_config_lookup_bool(config_t *cfg, char *name, int *value) 
{
	int val;
	int found_val = 0;
...
int
my_config_lookup_valid_bool(config_t *cfg, 
                     char *name, 
                     int *value, 
                     int validity_check, ...)
{
	return my_config_lookup_bool(cfg, name, value);
}


int
my_config_lookup_int(config_t *cfg, char *name, int *value)
{
	int val;
	int found_val = 0;
...
int
my_config_lookup_valid_int(config_t *cfg, 
                           char *name, 
                           int *value, 
                           int validity_check, ...)
{
	...
	if (my_config_lookup_int(cfg, name, &val) == CONFIG_TRUE) {
...
int
my_config_lookup_string(config_t *cfg, char *name, char **value)
{
	char *val;
	int  found_val = 0;
...
int
my_config_lookup_valid_string(config_t *cfg, 
                              char *name, 
                              char **value, 
                              int validity_check, ...)
{
	...
	if (my_config_lookup_string(cfg, name, &val) == CONFIG_TRUE) {
		switch (validity_check) {
			case CONFIG_NO_CHECK:
				*value = val;
				return CONFIG_TRUE;
