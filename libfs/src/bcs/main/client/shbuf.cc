...
int SharedBuffer::SignalReader(bool lock)
{
    int ret;
    int r;
    ...
    if ((ret = ipc_->call(::SharedBuffer::Protocol::kConsume, ipc_->id(), id_, r)) < 0)
    {
        ret = -E_IPC;
        goto done;
    }
 ...
 */
int SharedBuffer::Write(const char* src, size_t n)
{
