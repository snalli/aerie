...
void FileSystemObjectManager::Register(int type_id, const char* type_str,
                                       SuperBlockFactory* sb_factory, InodeFactory* inode_factory)
{
...
void FileSystemObjectManager::Register(SuperBlockFactory* sb_factory, InodeFactory* inode_factory)
{
    return Register(sb_factory->TypeID(), sb_factory->TypeStr().c_str(), sb_factory, inode_factory);
}

void FileSystemObjectManager::Unregister(int /*type_id*/)
{
    // TODO
    return;
}

int FileSystemObjectManager::FSTypeStrToId(const char* fs_type)
{
...
int FileSystemObjectManager::CreateSuperBlock(Session* session, int fs_type, SuperBlock** sbp)
{
...
int FileSystemObjectManager::CreateSuperBlock(Session* session, const char* fs_type,
                                              SuperBlock** sbp)
{
...
int FileSystemObjectManager::LoadSuperBlock(Session* session, osd::common::ObjectId oid,
 ...
 *
 */
int FileSystemObjectManager::CreateInode(Session* session, Inode* parent, int inode_type,
                                         Inode** ipp)
{
    int ret;
    ...
    Inode* ip;
    int fs_type = parent->fs_type(); // insight : How many types of filesystem are there ? Ans :
                                     // Guess : pxfs, kvfs ??
    ...
    // insight : Parent determines fs_type. fs_type determines iterator. Iterator determines
    // at how the arguments to the function flow. This function had 4.
 ...
 *
 */
int FileSystemObjectManager::DestroyInode(Session* session, Inode* ip)
{
    int ret;
    InodeFactoryMap::iterator it;
    InodeFactory* inode_factory;
