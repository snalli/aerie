...
class fast_rpc
{
  protected:
    ...
    void* map_shared_file(string filename, int* fdret, int type = OTHER)
    {
        ...
        channel = mmap(0, filesize, PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0);
    ...
    inline void rpcwait_incoming(volatile unsigned* sh_chan)
    {
        ...
        // spin on the signal
        while (*sh_chan != 1)
            __asm__ volatile("" ::: "memory"); // compiler barrier — prevent hoisting
    }
    ...
    inline unsigned enqueue_send_and_wait(rpc_sync_t* rpc_sync)
    {
        ...
        case UNUSED:
            assert(
                __sync_bool_compare_and_swap(&(rpc_signal[myticket].status), UNUSED, INUSE_NOCONT));
        ...
        case INUSE_NOCONT:
            ...
            assert(__sync_bool_compare_and_swap(&(rpc_signal[myticket].status), INUSE_NOCONT,
                                                INUSE_CONT));
            break;
        case INUSE_CONT:
    ...
    int sendMsg(rpc_sync_t* rpc_sync, char* msg, unsigned sizeInChar, char*& ret_buf, int& ret_sz)
    {
        ...
        if (chkChksum(mybuff->data, mybuff->sizeInChar,
                      calcChksum(mybuff->data, mybuff->sizeInChar)))
        {
...
class rpcc : public fast_rpc
{
  ...
  public:
    ...
    int bind();
    int unbind();
    int call1(unsigned int proc, marshall& req, unmarshall& rep);
    ...
    template <class R> int call(unsigned int proc, R& r);
    template <class R, class A1> int call(unsigned int proc, const A1& a1, R& r);
    ...
    int call(unsigned int proc, const A1& a1, const A2& a2, const A3& a3, const A4& a4,
             const A5& a5, R& r);
...
template <class R> int rpcc::call_m(unsigned int proc, marshall& req, R& r)
{
    unmarshall u;
...
template <class R> int rpcc::call(unsigned int proc, R& r)
{
    marshall m;
    return call_m(proc, m, r);
}

template <class R, class A1> int rpcc::call(unsigned int proc, const A1& a1, R& r)
{
    marshall m;
    m << a1;
    return call_m(proc, m, r);
...
template <class R, class A1, class A2>
int rpcc::call(unsigned int proc, const A1& a1, const A2& a2, R& r)
{
    marshall m;
    m << a1;
    m << a2;
...
template <class R, class A1, class A2, class A3>
int rpcc::call(unsigned int proc, const A1& a1, const A2& a2, const A3& a3, R& r)
{
    marshall m;
    m << a1;
    m << a2;
...
template <class R, class A1, class A2, class A3, class A4>
int rpcc::call(unsigned int proc, const A1& a1, const A2& a2, const A3& a3, const A4& a4, R& r)
{
    marshall m;
    m << a1;
    m << a2;
...
template <class R, class A1, class A2, class A3, class A4, class A5>
int rpcc::call(unsigned int proc, const A1& a1, const A2& a2, const A3& a3, const A4& a4,
               const A5& a5, R& r)
{
    marshall m;
    m << a1;
...
#define MAX_LOG_FILE_SIZE_PT MAX_LOG_FILE_SIZE / NUMTHREADS
  ...
  public:
    ...
    void reg(unsigned int proc, S*,
             int (S::*meth)(const A1, const A2, const A3, const A4, const A5, R& r));
    template <class S, class A1, class A2, class A3, class A4, class A5, class A6, class R>
    void reg(unsigned int proc, S*,
             int (S::*meth)(const A1, const A2, const A3, const A4, const A5, const A6, R& r));
    template <class S, class A1, class A2, class A3, class A4, class A5, class A6, class A7,
              class R>
    void reg(unsigned int proc, S*,
             int (S::*meth)(const A1, const A2, const A3, const A4, const A5, const A6, const A7,
                            R& r));
};
...
template <class S, class A1, class A2, class A3, class A4, class A5, class R>
void rpcs::reg(unsigned int proc, S* sob,
               int (S::*meth)(const A1 a1, const A2 a2, const A3 a3, const A4 a4, const A5 a5,
                              R& r))
{
    class h1 : public handler
    {
      ...
      public:
        h1(S* xsob,
           int (S::*xmeth)(const A1 a1, const A2 a2, const A3 a3, const A4 a4, const A5 a5, R& r))
            : sob(xsob), meth(xmeth)
        {
        }
        int fn(unmarshall& args, marshall& ret)
                              ...
                              const A6 a6, R& r))
{
    class h1 : public handler
    {
      private:
        S* sob;
        int (S::*meth)(const A1 a1, const A2 a2, const A3 a3, const A4 a4, const A5 a5, const A6 a6,
                       R& r);

      public:
        h1(S* xsob, int (S::*xmeth)(const A1 a1, const A2 a2, const A3 a3, const A4 a4, const A5 a5,
                                    const A6 a6, R& r))
            : sob(xsob), meth(xmeth)
        {
        }
