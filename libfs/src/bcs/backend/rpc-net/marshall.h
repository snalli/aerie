...
template <class A, class B> marshall& operator<<(marshall& m, const std::map<A, B>& d)
{
...
template <class A, class B> unmarshall& operator>>(unmarshall& u, std::map<A, B>& d)
{
    unsigned int n;
    u >> n;
