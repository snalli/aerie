...
//#include "../../libfs/src/common/hrtime.h"
...
flowoplib_filesetup(threadflow_t *threadflow, flowop_t *flowop,
    fbint_t *wssp, fb_fdesc_t **fdescp, off64_t* fsize)
{
	...
	if ((fd > 0) && (threadflow->tf_fse[fd] != NULL)) {
		...
		if(fsize != NULL)
                        *fsize = threadflow->tf_fse[fd]->fse_size;
    ...
    fbint_t *wssp, caddr_t *iobufp, fb_fdesc_t **filedescp, fbint_t iosize)
{
	...
	if ((ret = flowoplib_filesetup(threadflow, flowop, wssp, filedescp, NULL)) !=
	    FILEBENCH_OK)
		return (ret);
...
flowoplib_read(threadflow_t *threadflow, flowop_t *flowop)
{
	...
	if (avd_get_bool(flowop->fo_random)) {
		...
		//filebench_log(LOG_VERBOSE,"reading %d getting %d", iosize, ret);
...
unsigned long long int total_expected = 0, total_obtained = 0;
...
flowoplib_appendfilerand(threadflow_t *threadflow, flowop_t *flowop)
{
	...
	/* Measure time to write bytes */
	flowop_beginop(threadflow, flowop);
	/* KVFS AND KVS don't support appends so we do it ourselves */
	#ifdef KVFS
		/* 1.read */
