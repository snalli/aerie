...
class StorageSystem
{
  public:
    StorageSystem(::server::Ipc* ipc, StoragePool* pool)
        : ipc_(ipc), salloc_(NULL), pool_(pool), can_commit_suicide_(false)
    {
        pthread_mutex_init(&mutex_, NULL);
    }

    osd::cc::server::HLockManager* hlckmgr()
    {
        return hlckmgr_;
    }
    osd::cc::server::LockManager* lckmgr()
    {
        return lckmgr_;
    }
    ...
    {
        return StorageSystemDescriptor(super_obj_->oid(), session->shbuf_->Descriptor());
    }
...
template <typename Session> class StorageSystemT : public StorageSystem
{
  public:
    // factory methods
    static int Load(::server::Ipc* ipc, const char* source, unsigned int flags,
                    StorageSystemT** storage_system_ptr);
    static int Make(const char* target, unsigned int flags);
    static int Make(const char* target, unsigned int flags, int root_obj_size, void** super_objp,
                    void** root_obj);

    StorageSystemT(::server::Ipc* ipc, StoragePool* pool) : StorageSystem(ipc, pool)
    {
    }
    ...
    int Mount(int clt, const char* source, unsigned int flags,
    ...
    class IpcHandlers
    {
      public:
        ...
        int Mount(unsigned int clt, std::string source, unsigned int flags,
                  StorageSystemProtocol::MountReply& rep);
...
template <typename Session> int StorageSystemT<Session>::Init()
{
    int ret;
    ...
    if ((ret = ipc_->shbuf_manager()->RegisterSharedBufferType("OsdSharedBuffer",
                                                               OsdSharedBuffer::Make)) < 0)
    {
        return ret;
    }
    return E_SUCCESS;
...
int StorageSystemT<Session>::Load(::server::Ipc* ipc, const char* source, unsigned int /*flags*/,
                                  StorageSystemT** storage_system_ptr)
{
    int ret;
    void* b;
    ...
    DBG_LOG(DBG_INFO, DBG_MODULE(server_storagesystem), "Load storage system %s\n", source);
    ...
    if ((storage_system->super_obj_ = osd::containers::server::SuperContainer::Object::Load(b)) ==
        NULL)
    {
        return -E_NOMEM;
    }
 ...
 */
template <typename Session>
int StorageSystemT<Session>::Make(const char* target, unsigned int /*flags*/, int root_obj_size,
                                  void** super_objp, void** root_obj)
{
    osd::containers::server::SuperContainer::Object* super_obj;
    ...
    master_extent_size +=
        sizeof(osd::containers::server::SetContainer<osd::common::ObjectId>::Object);
    ...
    *root_obj = (void*) b;
    // storage container
    b += root_obj_size;
    if ((set_obj = osd::containers::server::SetContainer<osd::common::ObjectId>::Object::Make(
             session, b)) == NULL)
    {
        return -E_NOMEM;
    }
 ...
 */
template <typename Session>
int StorageSystemT<Session>::Make(const char* target, unsigned int /*flags*/)
{
    osd::containers::server::SuperContainer::Object* super_obj;
...
template <typename Session> int StorageSystemT<Session>::Close()
{
    int ret;
...
template <typename Session> StorageSystemT<Session>::~StorageSystemT()
{
    delete registry_;
    delete salloc_;
    delete hlckmgr_;
...
template <typename Session>
int StorageSystemT<Session>::Mount(int clt, const char* /*source*/, unsigned int /*flags*/,
                                   StorageSystemProtocol::MountReply& rep)
{
    int ret;
    Session* session;
...
template <typename Session>
int StorageSystemT<Session>::IpcHandlers::Register(StorageSystemT* module)
{
    module_ = module;
...
template <typename Session>
int StorageSystemT<Session>::IpcHandlers::Mount(unsigned int clt, std::string source,
                                                unsigned int flags,
                                                StorageSystemProtocol::MountReply& rep)
{
    int ret;
