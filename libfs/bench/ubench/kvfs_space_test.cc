		...
		if (ret > 0) {
			...
			if (stats->successful_writes % 10 == 0) {
				...
				printf("  [%3zu%%] Written: %zu bytes, %d keys\n",
					   percent, stats->bytes_written, stats->num_keys);
			}
		} else {
...
int main(int argc, char* argv[])
{
	...
	while ((ch = getopt(argc, argv, "h:v:m:")) != -1) {
		switch (ch) {
			...
			case 'v': stats.value_size = atoi(optarg);         break;
			case 'm': stats.max_size_mb = atoi(optarg);        break;
			default:  break;
		}
	}
	...
	printf("Total bytes written: %zu bytes (%.2f MB)\n",
		   stats.bytes_written, (double)stats.bytes_written / (1024*1024));
