...
fb_lfs_pread(fb_fdesc_t *fd, caddr_t iobuf, fbint_t iosize, off64_t fileoffset)
{
	int ret;
	#ifdef LIBFS
	ret = libfs_pread(fd->fd_num, (void *)iobuf, (size_t)iosize, (off_t)fileoffset);
	#elif RXFS
	ret = rxfs_pread(fd->fd_num, (void *)iobuf, (size_t)iosize, (off_t)fileoffset);
	#elif RXFS_F
	ret = rxfs_pread(fd->fptr, (void *)iobuf, (size_t)iosize, (off_t)fileoffset);
	#else
	ret = (pread64(fd->fd_num, iobuf, iosize, fileoffset));
	#endif
	return ret;
...
{
	#ifdef LIBFS
	fd->fd_num = libfs_open(path, flags);
	if(fd->fd_num > 0)
		return FILEBENCH_OK;
	#elif RXFS
	fd->fd_num = rxfs_open(path, flags);
	if(fd->fd_num > 0)
		return FILEBENCH_OK;
	#elif RXFS_F
...
/*
 * Does an unlink (deeete) of a file.
 */
static int
fb_lfs_unlink(char *path)
{
	...
	//printf("delete done\n");
	#elif KVS
	return (kvs_del(path+PREFIXIGNORE));
	#else
	return (unlink(path));
	#endif
}
...
{
	#ifdef LIBFS
	return (libfs_fsync(fd->fd_num));
	#elif RXFS
	return (rxfs_fsync(fd->fd_num));
	#else
...
fb_lfs_mkdir(char *path, int perm)
{
	...
	#elif RXFS
	// the directories are created during preallocation which is done
	// by libfs
	ret = (libfs_mkdir(path, 0));
	#elif RXFS_F
	ret = (libfs_mkdir(path, 0));
...
fb_lfs_recur_rm(char *path)
{
	...
	// LibFS - Wipe off the entire storage
	//(void) snprintf(cmd, sizeof (cmd), "./scratch/nvm/stamnos/libfs/build/src/pxfs/tool/pxfs create -p //tmp//stamnos_pool -s 256M -t mfs");
	/*
...
fb_lfs_stat(char *path, struct stat64 *statbufp)
{
	...
	#ifdef LIBFS
	ret = libfs_stat(path, statbufp);
	#elif RXFS
	ret = rxfs_stat(path, statbufp);
	#elif RXFS_F
	ret = rxfs_stat(path, statbufp);
	#else
	ret = (stat64(path, statbufp));
	#endif 
	if(!ret)
		return 0;
	else
...
fb_lfs_write(fb_fdesc_t *fd, caddr_t iobuf, fbint_t iosize)
{
	...
	return iosize;
	#elif KVS
	ret = kvs_put(fd->fname, iobuf, iosize);
	#else
