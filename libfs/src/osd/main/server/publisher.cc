...
Publisher::Publisher(::server::Ipc* ipc) : ipc_(ipc)
{
...
int Publisher::Init()
{
    RegisterOperation(osd::Publisher::Message::LogicalOperation::kAllocContainer,
                      Publisher::AllocContainer);
    if (ipc_)
    {
...
int Publisher::RegisterOperation(int lgc_op_id, LogicalOperation lgc_op)
{
...
int Publisher::Publish(OsdSession* session)
{
    int ret;
    char buf[512];
    int n;
...
int Publisher::AllocContainer(::osd::server::OsdSession* session, char* buf,
                              ::osd::Publisher::Message::BaseMessage* /*next*/)
{
    osd::common::ObjectId set_oid;

    ::osd::Publisher::Message::LogicalOperation::AllocContainer* lgc_op =
        LoadLogicalOperation<::osd::Publisher::Message::LogicalOperation::AllocContainer>(session,
                                                                                          buf);
    ...
    return session->salloc()->AllocateContainerFromSet(session, set_oid, lgc_op->oid_,
                                                       lgc_op->index_hint_);
}

int Publisher::IpcHandlers::Register(Publisher* publisher)
{
...
int Publisher::IpcHandlers::Publish(int clt, int& /*unused*/)
{
    int ret;
