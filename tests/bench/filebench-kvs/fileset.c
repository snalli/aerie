...
fileset_alloc_file(filesetentry_t *entry)
{
	...
	filebench_log(LOG_DEBUG_IMPL, "Populated %s", entry->fse_path);
	filebench_log(LOG_DEBUG_IMPL, "Reuse %d", entry->fse_flags & FSE_REUSING);
	...
	//filebench_log(LOG_VERBOSE,"file size %d", entry->fse_size);
...
extern int libstat(char *, struct stat64 *);
...
fileset_populate_file(fileset_t *fileset, filesetentry_t *parent, int serial)
{
	...
	double drand;
	uint_t index;
	unsigned long long int drand_rnd = 0;
