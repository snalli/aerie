...
{
  public:
    IntervalKey(const int low, const int high) : low_(low), high_(high)
    {
    }
...
class Interval
{
  public:
    Interval(ByteContainer::Object* obj, const int low, const int high)
        : low_(low), high_(high), object_(obj)
    {
...
class ByteContainer::VersionManager : public osd::vm::client::VersionManager<ByteContainer::Object>
{
  ...
  public:
    VersionManager()
    {
        printf("\n @ Inside ByteContainer::VersionManager. from "
               "src/osd/containers/byte/container-shadow.h");
    }
