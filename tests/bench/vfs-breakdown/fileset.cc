 ...
 */
static char* trunc_dirname(char* dir)
{
 ...
 */
int mkdir_r(std::string path, int mode)
{
    char tmp[256];
    char* p = NULL;
    size_t len;
...
FileSet::FileSet(std::string path, int nfiles, int dirwidth_inner, int dirwidth_leaf,
                 FileState initial_state)
{
    ...
    while (bfs.size())
    {
        ...
        if (cur_entry->_level == depth)
        {
            ...
            for (int i = 0; i < dirwidth_leaf; i++)
            {
                ...
                Entry* entry =
                    new Entry(Entry::kFile, ss.str(), cur_entry->_level + 1, initial_state);
                fidx->insert(entry);
                files_count++;
            }
        ...
        else
        {
            ...
            for (int i = 0; i < dirwidth_inner; i++)
            {
                ...
                ss << "/";
                Entry* entry =
                    new Entry(Entry::kDir, ss.str(), cur_entry->_level + 1, initial_state);
                fidx->insert(entry);
                bfs.push(entry);
            }
        }
...
void FileSet::create_files(int file_size)
{
    ...
    int i = 0;
    for (itr = next = _index[kNotExists]->_map.begin(); itr != _index[kNotExists]->_map.end();
         itr = next)
    {
...
FileSet::Entry::Entry(Entry::Type type, std::string name, int level, FileState initial_state)
    : _type(type), _name(name), _level(level), _state(initial_state)
{
}
