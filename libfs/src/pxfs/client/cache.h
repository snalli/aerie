...
// #define CONFIG_DEBUG  	// prints out a log of all inserts/deletes from cache and
// opens/writes/reads/close to debug.txt #define FAST_SPIDER_LOCK_PATH #define MAX_INST 400000 	//
// sets the upper limit on the cache. Set it high enough so that we never reach it #define
// LATENCY_PRESENT	// Enables/disable latency on writes to SCM device
...
class Cache
{
    ...
    struct Dentry
    {
        Dentry() : self(NULL), parent(NULL)
        {
        }

        Dentry(void* self_, void* parent_) : self(self_), parent(parent_)
        {
        }
        void* self;
        void* parent;
