...
rpcc::rpcc(sockaddr_in d, bool retrans)
    : dst_(d), srv_nonce_(0), bind_done_(false), xid_(1), lossytest_(0), retrans_(retrans),
      reachable_(true), chan_(NULL), destroy_wait_(false)
{
    ...
    DBG_LOG(DBG_DEBUG, DBG_MODULE(rpc), "rpcc::rpcc cltn_nonce is %d lossy %d\n", clt_nonce_,
            lossytest_);
}
...
rpcc::~rpcc()
{
    DBG_LOG(DBG_DEBUG, DBG_MODULE(rpc), "rpcc::~rpcc delete nonce %d channo=%d\n", clt_nonce_,
...
int rpcc::call1(unsigned int proc, marshall& req, unmarshall& rep, TO to)
{
    ...
    jsl_log(JSL_DBG_2, "rpcc::call1 %u call done for req proc %x xid %u %s:%d done? %d ret %d \n",
            clt_nonce_, proc, ca.xid, inet_ntoa(dst_.sin_addr), ntohs(dst_.sin_port), ca.done,
            ca.intret);
...
void rpcs::dispatch(djob_t* j)
{
    ...
    case NEW: // new request
        ...
        jsl_log(JSL_DBG_2,
                "rpcs::dispatch: sending and saving reply of size %d for rpc %u, proc %x ret %d, "
                "clt %u\n",
...
marshall& operator<<(marshall& m, long long int x)
{
    m << (unsigned int) (x >> 32);
    m << (unsigned int) x;
    return m;
...
unmarshall& operator>>(unmarshall& u, long long int& x)
{
