...
class ObjectManager : public osd::client::ObjectManagerOfType
{
  public:
    ...
    void Close(OsdSession* session, ObjectId oid, bool update)
    {
        ...
        (void) r1;
        int r2 =
            static_cast<ObjectProxy<Subject, VersionManager>*>(obj2_proxy)->vClose(session, update);
        assert(r2 == E_SUCCESS);
        (void) r2;
    }
    ...
    void CloseAll(OsdSession* session, bool update, bool flush = false)
    {
        ...
        for (itr = oid2obj_map_.begin(); itr != oid2obj_map_.end(); itr++)
        {
            ...
            int r = static_cast<ObjectProxy<Subject, VersionManager>*>(obj2_proxy)
                        ->vClose(session, update, flush);
            assert(r == E_SUCCESS);
            (void) r;
        }
    }
...
{
  public:
    ObjectProxyReference(void* owner = NULL) : osd::common::ObjectProxyReference(owner)
    {
    }
 ...
 */
template <class Subject, class VersionManager>
class ObjectProxy : public osd::vm::client::ObjectProxy<ObjectProxy<Subject, VersionManager>,
                                                        Subject, VersionManager>
{
  ...
  public:
    ObjectProxy(OsdSession* session, ObjectId oid)
        : osd::vm::client::ObjectProxy<ObjectProxy<Subject, VersionManager>, Subject,
                                       VersionManager>(session, oid),
          session_(session)
    {
        // insight : DO ME : verify if this is the ObjectProxy constructor getting called when
    ...
    int Lock(OsdSession* session, lock_protocol::Mode mode)
    {
        ...
        assert((osd::vm::client::ObjectProxy<ObjectProxy<Subject, VersionManager>, Subject,
                                             VersionManager>::vOpen() == 0));
             ...
             int flags = 0)
    {
        ...
        if ((ret = osd::cc::client::ObjectProxy::Lock(session, parent, mode, flags)) !=
            lock_protocol::OK)
        {
            return ret;
        }
    ...
    // ~ObjectProxy() { printf("\n + Destroying ObjectProxy 004. in
    // src/osd/main/client/rwproxy.h");}
