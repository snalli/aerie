...
bool connection::readpdu()
{
    if (!rpdu_.sz)
    {
        ...
        if (sz > MAX_PDU)
        {
            ...
            jsl_log(JSL_DBG_2,
                    "connection::readpdu read pdu TOO BIG %d network order=%x %x %x %x %x\n", sz,
...
connection* connect_to_dst(const sockaddr_in& dst, chanmgr* mgr, int lossy)
{
    ...
    //[5866] CRITICAL in connect_to_dst <build/src/bcs/backend/rpc-net/connection.cc,408>:
    // rpcc::connect_to_dst failed to 127.0.0.1:10000
    //
    if (connect(s, (sockaddr*) &dst, sizeof(dst)) < 0)
    {
        jsl_log(JSL_DBG_1, "rpcc::connect_to_dst failed to %s:%d\n", inet_ntoa(dst.sin_addr),
                (int) ntohs(dst.sin_port));
        close(s);
        return NULL;
    }
    jsl_log(JSL_DBG_2, "connect_to_dst fd=%d to dst %s:%d\n", s, inet_ntoa(dst.sin_addr),
            (int) ntohs(dst.sin_port));
