...
public:
	FileSystemManager(Ipc* ipc, ssa::server::Dpo* ssa);
	int Init();
	...
	int MountFileSystem(int clt, const char* source, const char* target, int fs_type, unsigned int flags, ssa::common::ObjectId* oid); 
	int MountFileSystem(int clt, const char* source, const char* target, const char* fs_type, unsigned int flags, ssa::common::ObjectId* oid); 

	class IpcHandlers {
	public:
		int Register(FileSystemManager* module);
		int CreateFileSystem(unsigned int clt, std::string target, std::string fs_type, unsigned int nblocks, unsigned int block_size, unsigned int flags, ssa::common::ObjectId& r);
		int MountFileSystem(unsigned int clt, std::string source, std::string target, std::string fs_type, unsigned int flags, ssa::common::ObjectId& r);
