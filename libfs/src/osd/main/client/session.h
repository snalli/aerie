...
class OsdSession
{
  public:
    OsdSession(osd::client::StorageSystem* stsystem)
        : stsystem_(stsystem), lckmgr_(stsystem->lckmgr()), hlckmgr_(stsystem->hlckmgr()),
          salloc_(stsystem->salloc()), omgr_(stsystem->omgr())
    {
        journal_ = new osd::client::Journal(this);
    }

    osd::client::StorageAllocator* salloc()
    {
        return salloc_;
    }
    osd::client::Journal* journal()
    {
        return journal_;
    }
    osd::client::StorageSystem* stsystem()
    {
        return stsystem_;
    }
    osd::client::ObjectManager* omgr()
    {
        return stsystem_->omgr();
    };
