...
#ifndef __STAMNOS_OSD_LOCK_PROTOCOL_H
#define __STAMNOS_OSD_LOCK_PROTOCOL_H
...
class lock_protocol
{
  public:
    ...
    enum rpc_numbers
    {
        DEFINE_RPC_NUMBER(OSD_LOCK_PROTOCOL)
    };
};
...
class lock_protocol::Mode
{
  public:
    ...
    Mode() : value_(lock_protocol::Mode::NL)
    {
    }

    Mode(lock_protocol::Mode::Enum val) : value_(val)
    {
    }
    ...
    static int PartialOrder(lock_protocol::Mode mode1, lock_protocol::Mode mode2)
    {
        ...
        { /* s1 == s2 */
            if (mode1 == Mode(lock_protocol::Mode::XL) && mode2 == Mode(lock_protocol::Mode::IXSL))
            {
                return 1;
            }
    ...
    static lock_protocol::Mode Supremum(lock_protocol::Mode mode1, lock_protocol::Mode mode2)
    {
        int po = 0;
    ...
    static bool AbidesRecursiveRule(lock_protocol::Mode mode, lock_protocol::Mode recursive_mode)
    {
    ...
    static bool AbidesHierarchyRule(lock_protocol::Mode mode, lock_protocol::Mode ancestor_mode)
    {
...
class lock_protocol::Mode::Set
{
  public:
    ...
    Set(lock_protocol::Mode::Enum val) : value_(static_cast<int>(BITMAP_SET(val)))
    {
    }

    Set(lock_protocol::Mode mode) : value_(static_cast<int>(BITMAP_SET(mode.value_)))
    {
    }

    Set(const lock_protocol::Mode::Set& mode_set) : value_(mode_set.value_)
    {
    }

    Set(int val) : value_(val)
    {
    }
...
class lock_protocol::Mode::Set::Iterator
{
  public:
    ...
    Iterator(lock_protocol::Mode::Set set) : value_(set.value_)
    {
    }
...
#include "osd/main/common/lock_protocol-inl.h"

#endif // __STAMNOS_OSD_LOCK_PROTOCOL_H
