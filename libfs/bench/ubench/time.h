...
#define MEASURE_TIME_DIFF_USEC(runtime)                                                            \
    do                                                                                             \
    {                                                                                              \
        runtime = 1000000 * (time_stop.tv_sec - time_start.tv_sec) + time_stop.tv_usec -           \
                  time_start.tv_usec;                                                              \
    } while (0);
...
static std::string measure_time_summary(int numops, unsigned long long runtime,
                                        hrtime_t runtime_cycles)
{
    ...
    if (numops > 1)
    {
        ss << "avg_runtime    " << runtime / numops << " us (" << runtime / numops / 1000 << ")"
           << std::endl;
    }
