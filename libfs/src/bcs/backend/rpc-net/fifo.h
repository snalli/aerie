...
template <class T> fifo<T>::fifo(int limit) : max_(limit)
{
...
template <class T> bool fifo<T>::size()
{
    ScopedLock ml(&m_);
    return q_.size();
}

template <class T> bool fifo<T>::enq(T e, bool blocking)
{
    ScopedLock ml(&m_);
    while (1)
    {
...
template <class T> void fifo<T>::deq(T* e)
{
    ScopedLock ml(&m_);
