...
StorageAllocator::Load(::server::Ipc* ipc, StoragePool* pool, StorageAllocator** sallocp) 
{
	...
	if ((set_obj = osd::containers::server::SetContainer<osd::common::ObjectId>::Object::Load(freelist_oid)) == NULL) {
		return -E_NOMEM;
	}
...
int 
StorageAllocator::AllocateContainer(OsdSession* session, int type, int num, ::osd::StorageProtocol::ContainerReply& reply)
{
	osd::containers::server::SetContainer<osd::common::ObjectId>::Object* set_obj;
	...
	for (int i=0; i<freeset_->Size(); i++) {
		...
		if ((set_obj = osd::containers::server::SetContainer<osd::common::ObjectId>::Object::Load(set_oid)) == NULL) {
			break;
		}
	...
	// Create a new set
	extent_size = sizeof(osd::containers::server::SetContainer<osd::common::ObjectId>::Object);
	if ((ret = pool_->AllocateExtent(extent_size, (void**) &buffer)) < 0) {
		return ret;
	}
	if ((set_obj = osd::containers::server::SetContainer<osd::common::ObjectId>::Object::Make(session, buffer)) == NULL) {
		return -E_NOMEM;
	}
                                                 ...
                                                 ::osd::StorageProtocol::ContainerReply& r)
{
	...
	if ((ret = salloc_->AllocateContainer(static_cast<OsdSession*>(session), 
	                                      type, num, r)) < 0) {
		return -ret;
	}
	return E_SUCCESS;
