...
static int ut_barrier_init(ut_barrier_t* bp, int count, int pshared)
{
    int r;
    int i;
...
static int ut_barrier_wait(register ut_barrier_t* bp)
{
    register int sbi = bp->sbi;

    pthread_mutex_lock(&bp->sb[sbi].wait_lk);

    if (bp->sb[sbi].runners == 1)
...
static inline int ut_barrier_destroy(ut_barrier_t* bp)
{
    int r;
    int i;
