...
int FileSystem::Make(const char* target, size_t nblocks, size_t block_size, int flags)
{
    ...
    if ((ret = StorageSystem::Make(target, flags, root_obj_size, (void**) &super_obj, &root_obj)) <
        0)
    {
        return ret;
    }
...
int FileSystem::Mount(int clt, const char* source, unsigned int flags,
...
int FileSystem::IpcHandlers::Mount(unsigned int clt, std::string source, unsigned int flags,
                                   FileSystemProtocol::MountReply& rep)
{
    int ret;
